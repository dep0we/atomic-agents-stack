...
- **`[launch] Public-flip launch shape — README rewrite, comparison matrix, positioning, pre-flip readers`** ([#93](https://github.com/dep0we/atomic-agents-stack/issues/93)) — README rewrite anchored to positioning hooks (19 spec docs, vault portability, audit trail, provider neutrality), honest comparison matrix vs Letta/LangChain/LangGraph/AutoGen/openclaw, `/office-hours` pressure-test, and 5-reader pre-flip review loop.
