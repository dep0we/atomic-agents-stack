...
- **`[backend] LLMBackend Protocol — PR 1 of 4: canonical types + Protocol contract + registry`** ([#87](https://github.com/dep0we/atomic-agents-stack/issues/87)) — establishing the LLM provider abstraction layer in the protocol-pattern series alongside #60–#65 / #82–#84. **PR 1 ships the scaffolding only** — new `atomic_agents/llm/` package with six canonical `@dataclass(frozen=True)` types (`LLMToolDefinition`, `LLMToolUse`, `LLMToolResult`, `CacheDirective`, `LLMCapabilities`, `PricingInfo`), the `SyncLLMBackend` Protocol contract (`@runtime_checkable`), the process-local registry (`register_llm_backend` / `find_backend_for_model` with `preferred_provider` resolution + `AmbiguousBackendError`/`UnknownModelError` semantics), and `_RawLLMResponse` re-housed in `atomic_agents/llm/backend.py` with a new `reasoning_text` field reserved for #146. `atomic_agents._llm._RawLLMResponse` is now a re-export of the canonical class — same object across all import paths, no behavior change. 30 unit tests covering type construction, Protocol shape, registry register/get/iter, conflict resolution semantics, pickle roundtrip on `AmbiguousBackendError`, and the `_RawLLMResponse`-is-one-class invariant. No behavior change in this PR; nothing in the existing codebase imports the new module yet (PR 2 wires it). Opus subagent adversarial review caught a P1 (duplicate `_RawLLMResponse` class) + 3 P2s (pickle, empty-string `preferred_provider`, forward-references to a `model.md provider:` field that doesn't exist yet) + cheap P3 polish — all addressed pre-merge. Follow-up PRs: PR 2 `AnthropicLLMBackend` + `agent.py` tool dispatch refactor; PR 3 `OpenAICompatibleLLMBackend` + `MoonshotLLMBackend` + `model.md provider:` parser; PR 4 `docs/spec/31-llm-backend.md` + ~30-test conformance suite + README/CLAUDE.md/spec cross-refs. Will land across a future Minor release (additive; no `### BREAKING` callout). #148 (top_p/stop_sequences parity) filed inline as a follow-up.
