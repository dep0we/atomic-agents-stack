...
Filed as [#428](https://github.com/dep0we/atomic-agents-stack/issues/428). Shipped in 2 stages: PR 1 — SCAFFOLDING-ONLY carve (the queue cluster lifted out of `_cascade.py` into `atomic_agents/queue/` with zero new internal runtime callers wired; the documented spec/06 cron/project-runner free-function API preserved via a thin non-deprecated re-export shim); LOCK PR — spec/44 DRAFT→LOCKED with four targeted hardening commits (#476/#477/#478/#479) and 134 total tests. Runtime adoption (wiring into the cascade runner) deferred to [#469](https://github.com/dep0we/atomic-agents-stack/issues/469).
...
LOCKED spec/44 carries 12 normative MUSTs (base-8 PersonaBackend pattern + 4 queue-unique axes: atomic-claim-via-rename / no-double-claim-under-race; lease-expiry-recovery correctness; dead-letter terminal-transition / dead-work-stays-dead; `single_host_only` capability honesty). MUST 9 prose strengthened at LOCK with the lease_token no-clobber contract (callers MUST generate a fresh `lease_token` per session; collision on `claimed/<token>/` returns None). Conformance suite: 134 tests total — `tests/test_queue_backend_conformance.py` (58: parametrized conformance core + claim-race tests + doctor + export + import-equivalence) + `tests/test_queue_filesystem.py` (76: POSIX-rename + `.lease.json` sidecar + legacy-mtime-fallback + LOCK-hardening tests for #476/#477/#478/#479). LOCK-hardening summary: MUST 10 de-vacuoused + strip-RED control (#476); `export()` iterdir()-walk closes Python 3.13 rglob-follows-dir-symlinks DoS (#477); no-replace mkdir + O_EXCL claim probe closes lease_token-collision clobber (#478); `_write_no_follow` O_NOFOLLOW at all three sidecar sites closes sidecar-leaf perimeter escape (#479).
