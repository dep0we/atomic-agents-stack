...
**Batch embed cost gate (#544 PR1):** `PRIMITIVE_EMBED = 'embed'` (distinct from `PRIMITIVE_HELPER` — embedding bills from `EMBEDDING_PRICING`, not chat `PRICING`). Four JSONL triggers (`embed_batch_reservation`, `embed_batch_release`, `embed_reservation`, `embed_release`) all map to `PRIMITIVE_EMBED` in `_PRIMITIVE_BY_TRIGGER`. The batch gate at the `agent.call()` post-loop capture-commit site reserves worst-case cost (2x per_item_sum for batch/per-item fan-out), then emits a release in `finally` with actual cost (0 on `None`-return). `_has_effective_embed_cap()` resolves the full Policy+model.md+tree-cap hierarchy (NOT model.md-only) for the fail-closed predicate. `cost_estimated=True` never gates; only `CostReadResult.degraded AND effective_cap is not None` gates.

**Test layout:** `tests/stub_embedding.py` (TEST-ONLY `StubEmbeddingBackend` + `_RaisingStubEmbeddingBackend` for negative controls; never in production imports) + `tests/test_embedding_protocol_conformance.py` (60 conformance tests, parametrized over [stub, openai-mocked]) + `tests/test_openai_embedding.py` (53 OpenAI impl-specific tests; 3 skipped pending `ATOMIC_AGENTS_TEST_OPENAI_KEY`) + 21 cost tests added to `tests/test_costs.py` + 30 new tests in `tests/test_embed_cost_gate.py` (#544 PR1). 164 new tests total.
