...
Filed as [#428](https://github.com/dep0we/atomic-agents-stack/issues/428). Shipped in 2 stages: PR 1 — SCAFFOLDING-ONLY carve (the queue cluster lifted out of `_cascade.py` into `atomic_agents/queue/` with zero new internal runtime callers wired; the documented spec/06 cron/project-runner free-function API preserved via a thin non-deprecated re-export shim); LOCK PR — spec/44 DRAFT→LOCKED with four targeted hardening commits (#476/#477/#478/#479; the #477 export-walk hardening is defense-in-depth / version-independent, with a 3.13+ live-vector CI lane tracked in #595) and 136 total tests. Runtime adoption (wiring into the cascade runner) deferred to [#469](https://github.com/dep0we/atomic-agents-stack/issues/469).
