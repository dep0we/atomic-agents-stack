...
**Test layout:** `tests/stub_embedding.py` (TEST-ONLY `StubEmbeddingBackend` + `_RaisingStubEmbeddingBackend` for negative controls; never in production imports) + `tests/test_embedding_protocol_conformance.py` (60 conformance tests, parametrized over [stub, openai-mocked]) + `tests/test_openai_embedding.py` (53 OpenAI impl-specific tests; 3 skipped pending `ATOMIC_AGENTS_TEST_OPENAI_KEY`) + 21 cost tests added to `tests/test_costs.py` + 57 tests in `tests/test_embed_cost_gate.py` (39 from #544 PR1 + 18 from #544 PR2a) + 22 tests in `tests/test_corpus_query_embed_gate.py` (#544 PR2 CLI corpus-query gate). 212 new tests total.
