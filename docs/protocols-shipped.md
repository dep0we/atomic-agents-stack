...
Twelve backend protocols are locked for v1.0. A thirteenth, SecretBackend (#340), shipped in v2.0.0 with two reference implementations (FilesystemSecretBackend + GCPSecretManagerBackend) and LOCKED spec/38. A fourteenth, GoalBackend (#425 + #448 PR1/PR2/PR3 + #483 PR1 + #496 PR1 + #642 PR1 RE-LOCK), shipped in v2.0.0 with FilesystemGoalBackend and LOCKED spec/41 — RE-LOCKed at 13 MUSTs in #642 PR1 (arc-closer #448 PR3 locked the spec; #483 PR1 added clock injection + GoalManager thin shim + agent_root resolution + spec/41 normative addendum; #496 PR1 added backend-universe alignment — coordinator threads gate agent's log/policy/profile backends into OutcomeRunner; #642 PR1 RE-LOCK added create_goal()/list_goals() on GoalBackend Protocol, AddressableGoalBackend Protocol + for_goal() scope handle, GoalCapabilities.supports_multi_goal, GoalAlreadyExists exception, export() fail-loud guard while addressed goals present — multi-goal export deferred to #643). A fifteenth, OutcomeBackend (#426), shipped in v2.0.0 with FilesystemOutcomeBackend; write-path adopted in #448 PR2 and LOCKED spec/42. A sixteenth, JournalBackend (#427), shipped in v2.0.0 with FilesystemJournalBackend and LOCKED spec/43. A seventeenth, QueueBackend (#428), shipped in v2.0.0 with FilesystemQueueBackend and LOCKED spec/44 (LOCK PR includes four hardening commits: #476/#477/#478/#479; 137 tests; runtime adoption deferred to #469). An eighteenth, IdempotencyBackend (#520), shipped in v2.0.0 (PR 1 + PR 2 arc-closer) with FilesystemDedupLedger and LOCKED spec/45 — PR 2 wired the two-phase dedup gate into agent.call() (idempotency_key kwarg, lookup-before-lock COMPLETED short-circuit, begin-after-cost-gate, serve/queue/cron trigger integration, RunRecord audit fields, spec/22 versioned normative addendum). A nineteenth, EmbeddingBackend (#200 + #544 PR1/PR2a/PR2), shipped in v2.0.0 across PR 2 (Protocol + ref impl), PR 3 (registry + pgvector wiring + `input_type` kwarg), #544 PR1 (batch embed cost gate + PRIMITIVE_EMBED + doctor check + spec/20/22/34 addenda), #544 PR2a (cross-call `embed_cost` record + merge-write pre-read reservation), and #544 PR2 (CLI corpus-query embed gate + 4 gate-site normative MUSTs + direct-caller gate boundary + pgvector conformance fold + DRAFT→LOCKED) with OpenAIEmbeddingBackend and LOCKED spec/46. A twentieth, ConversationBackend (#535), shipped in v2.0.0 PR 1 + LOCK PR with FilesystemConversationBackend and LOCKED spec/47 — ships the Protocol + filesystem reference impl + full agent.call() wiring (three-channel backend selection, model-aware token budget, turn write-back gated on not _response_deferred, continuity_persisted=False on failure, conversation_id tagged on all 7 terminal JSONL sites) + SQLite/Postgres v2→v3 migration + check_conversation_backend() doctor check + spec/22 and spec/40 versioned normative addenda; PostgresConversationBackend deferred to Phase 3. A twenty-first, PrincipalBackend (#556), shipped in v2.0.0 PR 1 + LOCK PR with LocalPrincipalBackend + StaticClaimsPrincipalBackend and LOCKED spec/48 — ships the Protocol + two reference impls + HARD-REFUSE gate wired into agent.call() BEFORE the idempotency dedup lookup (MUST 10) + fail-closed opt-in serve HYBRID flow (raw identity header not trusted by default) + doctor check wired into run_doctor() with a fail-closed negative probe + DSN-credential redaction + a 12-MUST Implementer Contract. A security review (Claude adversarial + cross-family Codex) added four fail-closed fixes to the serve/principal seam: a cross-tenant collapse guard (an is_local_only backend left registered under perimeter-trust mints an unverified principal so the gate fires, instead of collapsing every tenant onto LOCAL_PRINCIPAL), ASCII-whitespace-only storage-key strip (Unicode-space subjects no longer collide — MUST 11), the full untruncated identity header as the authz subject (not the 512-char audit copy), and header-absent-under-perimeter fail-closed (unverified, not LOCAL_PRINCIPAL). LOCK ceremony added: full 12-MUST conformance-test map (MUST 2/3/9 object-identity tests); explicit run_id in principal-refused JSONL record; derive_storage_key made private. A raw-token self-verifying backend + role-based authorization deferred to later PRs. Each section captures the reference implementations shipped, the operator override surface, the doctor coherence check, the Implementer Contract location, and the architectural cliff the protocol closes.
...
**`workflow_id` cross-run correlation field (spec/22 versioned normative addendum, issue #622 PR1):** `RunRecord.workflow_id` optional field stamped on all 9 terminal JSONL record sites + helper + embed_cost records when `agent.call(workflow_id=...)` is set; threads through `delegate()` / `delegate_parallel()` into the child's `call()`; the coordinator's mirror record (`trigger='delegate'`, carries `delegated_agent`/`delegate_run_id`) is NOT stamped — count-once enforcement mechanism. `WorkflowSummary` dataclass + `aggregate_workflow(agents_root, workflow_id)` free function added to `dashboard/costs.py` + re-exported from `dashboard/__init__`; fans out via `discover_agents()`, queries each backend with `LogQuery(workflow_id=..., agent_name=agent)`, excludes the mirror via marker keys (NOT bare `trigger`), sums actor-cost records, OR-accumulates `cost_data_degraded`. `LogQuery.workflow_id` AND-predicate wired in all three backends (SQLite/Postgres `idx_workflow_id` partial index seek, Filesystem skip-predicate). SQLite + Postgres bumped to `_SCHEMA_VERSION = 4` with a `v3→v4` migration (adds `workflow_id TEXT` column + `idx_workflow_id` partial index `WHERE workflow_id IS NOT NULL`).
...
## GoalBackend (#425 + #448 PR1/PR2/PR3 + #483 PR1 + #496 PR1 + #642 PR1 RE-LOCK, LOCKED spec/41, the fourteenth)
...
**Reference implementation:** `FilesystemGoalBackend` (#425 scaffold + #448 PR1 write-path adoption + #448 PR2 CAS conformance + #448 PR3 coordinator arc-closer + #483 PR1 clock injection + GoalManager thin shim + agent_root resolution + #496 PR1 backend-universe alignment + #642 PR1 RE-LOCK). Reproduces today's exact `goal.md` / `goal_history.jsonl` / `goal_archive/` I/O byte-for-byte (except the intentional archive data-loss fix in #448 PR1 — optional fields are now preserved). Zero behavior change for home users outside that fix. As of #642 PR1, `FilesystemGoalBackend` also satisfies `AddressableGoalBackend` (`for_goal(goal_id)` scope handle) and exposes `create_goal()` / `list_goals()` — addressed run-goals live at `<agent_root>/goals/<goal_id>/`.

`FilesystemGoalBackend` stores goal state in `<agent_root>/goal.md` (frontmatter + body including the `## History` prose section) and structured history events in `<agent_root>/goal_history.jsonl` (JSONL, append-only). Archived goals live in `<agent_root>/goal_archive/<slug>.md` (flat `.md` file, no subdirectory). Addressed run-goals (added in #642 PR1) live at `<agent_root>/goals/<goal_id>/` with their own `goal.md` + `goal_history.jsonl`. The backend is `@runtime_checkable` and exposes `load_goal` / `save_goal` / `append_history_event` / `archive_goal(agent_id, reason="completed", when=None)` / `list_archived` / `read_schema_version` / `export` / `export_all` / `capabilities` plus the load-bearing `apply_transition(sub_goal_id, to_status, fields, *, expected_from_status=None, when=None)` atomic primitive (the `expected_from_status` CAS guard ships in #448 PR3; the `when: date | None = None` injectable clock on both `apply_transition` and `archive_goal` ships in #483 PR1 — `apply_transition`'s `when` controls the `## History` prose date prefix only, `archive_goal`'s `when` controls all four date-stamped fields, single-resolution); and the #642 PR1 additions `create_goal(agent_id, goal_id, goal, when=None)` (O_EXCL atomic creation, MUST 11) + `list_goals(agent_id)` (sorted enumeration including `'_standing'`, MUST 12). `FilesystemGoalBackend` also satisfies the separate `AddressableGoalBackend` Protocol via `for_goal(goal_id) -> GoalBackend` (MUST 13).
...
`doctor.check_goal_backend()` uses a three-probe pattern as of #642 PR1: `list_archived` liveness, then `list_goals` (asserts `list` return + `'_standing'` present when `goal.md` exists), then `load_goal` heavy probe — per `feedback_doctor_dual_probe_pattern` (extended to triple-probe). Detail dict includes `goal_ids` and `supports_multi_goal` as of #642. Located in `atomic_agents/doctor.py`.
...
LOCKED spec/41 carries 13 normative MUSTs after the #642 PR1 RE-LOCK (original 10 MUSTs unchanged; MUST 11 = `create_goal()` atomicity/collision semantics; MUST 12 = `list_goals()` enumeration; MUST 13 = `AddressableGoalBackend.for_goal()` routing — on the separate AddressableGoalBackend Protocol, not GoalBackend). The original 10 MUSTs: side-effect-free construction, capability honesty, `goal_text()` read-only, `save_goal()` write-what-I-give-you, round-trip fidelity, atomic `apply_transition` with enum-validate-fail-closed, archive write-ordering, collision-safe archive slug, idempotent archive retry, `apply_transition` compare-and-set CAS guard. Test suite: `tests/test_goal_backend_conformance.py` (60 tests, original 10 MUSTs; TEST 56–59 are the #483 PR1 clock-injection + ts-first-reorder + apply_transition prose-vs-ts conformance tests) + `tests/test_goal_multigoal_642.py` (74 collected, TEST 60–129 — TEST 113 parametrized ×5, so 70 TEST-number labels = 74 collected functions; MUST 11/12/13 + complete-on-partial recoverability + containment + doctor extension) + `tests/test_goal_filesystem.py` (filesystem-specific) + `tests/test_goal_doctor.py` (triple-probe, PASS/FAIL/SKIP ladder) + `tests/test_goal_dispatch_audit_ordering.py` (MUST 6 regression, 2 tests) + `tests/test_goal_coordinator.py` (coordinator integration, 21 tests — 19 from #448 PR3 + 2 backend-universe alignment tests added in #496 PR1: `test_coordinator_threads_gate_agent_backends_into_runner` instance-identity test + `test_coordinator_forwards_custom_log_backend_to_runner` behavioral test). The 4 pre-existing goal test files kept intact as the zero-behavior-change regression guard.
...
## JournalBackend (#427, LOCKED spec/43, the sixteenth)
...
## QueueBackend (#428, LOCKED spec/44, the seventeenth)

Filed as [#428](https://github.com/dep0we/atomic-agents-stack/issues/428). Shipped in 2 stages: PR 1 — SCAFFOLDING-ONLY carve (the queue cluster lifted out of `_cascade.py` into `atomic_agents/queue/` with zero new internal runtime callers wired; the documented spec/06 cron/project-runner free-function API preserved via a thin non-deprecated re-export shim); LOCK PR — spec/44 DRAFT→LOCKED with four targeted hardening commits (#476/#477/#478/#479; the #477 export-walk hardening is defense-in-depth / version-independent, with a 3.13+ live-vector CI lane tracked in #595) and 137 total tests. Runtime adoption (wiring into the cascade runner) deferred to [#469](https://github.com/dep0we/atomic-agents-stack/issues/469).
...
**Operator override:** `ATOMIC_AGENTS_QUEUE_BACKEND` env var (default `'filesystem'`). Factory `get_default_queue_backend(project_root)`; registry `register_queue_backend` / `get_queue_backend` / `list_queue_backends` / `unregister_queue_backend` in `atomic_agents/queue/__init__.py`. No operator CLI (deferred to a follow-up issue).
...
LOCKED spec/44 carries 12 normative MUSTs (base-8 PersonaBackend pattern + 4 queue-unique axes: atomic-claim-via-rename / no-double-claim-under-race; lease-expiry-recovery correctness; dead-letter terminal-transition / dead-work-stays-dead; `single_host_only` capability honesty). MUST 9 prose strengthened at LOCK with the lease_token no-clobber contract (callers MUST generate a fresh `lease_token` per session; collision on `claimed/<token>/` returns None). Conformance suite: 137 tests total — `tests/test_queue_backend_conformance.py` (62: parametrized conformance core + claim-race tests + non-FNF-rename placeholder-cleanup strip-RED controls + doctor + export + import-equivalence) + `tests/test_queue_filesystem.py` (75: POSIX-rename + `.lease.json` sidecar + legacy-mtime-fallback + LOCK-hardening tests for #476/#477/#478/#479). LOCK-hardening summary: MUST 10 de-vacuoused + strip-RED control (#476); `export()` explicit iterdir()-walk replaces `rglob('*')` with version-independent no-follow + per-subdir containment re-assertion (#477 — defense-in-depth against a future `recurse_symlinks` default; default `rglob` does not follow dir symlinks on 3.11-3.13, so verified-by-inspection today, 3.13+ live-vector lane tracked in #595); no-replace mkdir + O_EXCL claim probe closes lease_token-collision clobber (#478); `_write_no_follow` O_NOFOLLOW at all three sidecar sites closes sidecar-leaf perimeter escape (#479).
...
## EmbeddingBackend (#200 + #544, LOCKED spec/46, the nineteenth)

Filed as [#200](https://github.com/dep0we/atomic-agents-stack/issues/200). PR 2 shipped the Protocol + ref impl. PR 3 shipped the pgvector backends + registry + opt-in guard. [#544 PR1](https://github.com/dep0we/atomic-agents-stack/issues/544) shipped the live batch embed cost gate + PRIMITIVE_EMBED + doctor check + spec addenda. [#544 PR2a](https://github.com/dep0we/atomic-agents-stack/issues/544) shipped the cross-call embed accounting fix (dedicated `embed_cost` record with `cost_usd=actual_usd` visible to `sum_cost_for_period`) and the merge-write pre-read reservation fix (`read_note(merge_into)` before the gate, sized from the PRESERVED TARGET body — a merge preserves the body verbatim, so the backend re-embeds the target body alone, not the small fragment — Principle #4 enforce-before-pay). [#544 PR2](https://github.com/dep0we/atomic-agents-stack/issues/544) shipped the **CLI corpus-query embed gate** (`_corpus_query` emits `embed_reservation` + `embed_release` + `embed_cost` in a try/finally block; `--critical` skips headroom enforcement but still emits all audit records; cost-headroom check mirrors `dream._check_cap`; single-call token estimate `ceil(utf8_bytes / 3)`, no fan-out, priced from the model_id resolved off `EmbeddingCapabilities.embedding_backend_resolved`), **four gate-site normative MUSTs** (caller-side: pre-call reservation, release-in-finally, embed_cost-for-cross-call-accounting, fail-closed-predicate — categorically separate from the 9-MUST backend Implementer Contract, which stays at 9), the **direct-caller gate boundary** (`memory.search()` / `corpus.query()` on pgvector backends are ungated direct callers by design — the `llm_backend.chat()` analogy — with a future gated helper tracked at #586), the **pgvector conformance fold** (`PgvectorMemoryBackend` + `PgvectorCorpusBackend` exercised against the shared conformance suites via a deterministic `StubEmbeddingBackend`, gated `@requires_postgres`), and the **DRAFT→LOCKED ceremony**.
...
**Registry (#200 PR3):** `ATOMIC_AGENTS_EMBEDDING_BACKEND` env var enables semantic search (opt-in default: OFF). Constructor-injected by consuming backends OR resolved via `get_default_embedding_backend()`.

**Batch embed cost gate (#544 PR1):** `PRIMITIVE_EMBED = 'embed'` (distinct from `PRIMITIVE_HELPER` — embedding bills from `EMBEDDING_PRICING`, not chat `PRICING`). Five JSONL triggers (`embed_batch_reservation`, `embed_batch_release`, `embed_reservation`, `embed_release`, `embed_cost`) all map to `PRIMITIVE_EMBED` in `_PRIMITIVE_BY_TRIGGER`. The batch gate at the `agent.call()` post-loop capture-commit site reserves worst-case cost (2x per_item_sum for batch/per-item fan-out), then emits a release in `finally` with actual cost (per-written-note byte-token estimate; 0 only when no notes were written). `actual_usd` is NOT conditioned on whether the underlying `embed()` returned `None` — `write_note()` does not surface that signal, so a successfully-written note is charged the full per-note estimate (see the spec/22 and spec/46 addenda). `_has_effective_embed_cap()` resolves the full Policy+model.md+tree-cap hierarchy (NOT model.md-only) for the fail-closed predicate. `cost_estimated=True` never gates; only `CostReadResult.degraded AND effective_cap is not None` gates.

**Cross-call embed accounting (#544 PR2a):** A dedicated `embed_cost` JSONL record (`cost_usd=actual_usd`, `cost_source='actor'`, `model`, `cost_estimated`, `parent_run_id`, `parent_agent`) is emitted after `embed_batch_release`, conditioned on `actual_usd > 0`. This is the ONLY embed trigger with `cost_usd` — the release record remains audit-only. `sum_cost_for_period` now folds prior embed spend into the gate baseline on subsequent calls. Merge-write pre-read fix: a merge PRESERVES the target body verbatim (the backend re-embeds the stored TARGET body alone; the fragment lands in `sources` metadata, NOT the body), so the reservation and true-up loops now call `read_note(merge_into)` before the write loop and size the estimate from the PRESERVED TARGET body (PR1 sized from the small fragment, under-charging large merges); an over-cap merge is refused before billing (Principle #4). On read failure falls back to fragment-only with a WARNING. The `embed_cost` record shape is documented in a spec/22 versioned normative addendum (§"`embed_cost` cross-call accounting record") — a description-of-shipped-behavior addendum following the spec/45 PR2 (#520) / spec/22 read-failure (#497) precedent, not a normative-contract change; it corrects the PR1 addendum's now-superseded "cost_usd ABSENT / deferred to PR2" statement.

**Test layout:** `tests/stub_embedding.py` (TEST-ONLY `StubEmbeddingBackend` + `_RaisingStubEmbeddingBackend` for negative controls; never in production imports) + `tests/test_embedding_protocol_conformance.py` (60 conformance tests, parametrized over [stub, openai-mocked]) + `tests/test_openai_embedding.py` (53 OpenAI impl-specific tests; 3 skipped pending `ATOMIC_AGENTS_TEST_OPENAI_KEY`) + 21 cost tests added to `tests/test_costs.py` + 57 tests in `tests/test_embed_cost_gate.py` (39 from #544 PR1 + 18 from #544 PR2a) + 24 tests in `tests/test_corpus_query_embed_gate.py` (#544 PR2 CLI corpus-query gate). 215 new tests total.

**Doctor (#544 PR1):** `check_embedding_backend()` wired into `run_doctor()` with SKIP/PASS/WARN/FAIL branches. No billable embed() probe. WARN when `_api_key is None` (backend configured but key missing — embeds degrade). SKIP when `ATOMIC_AGENTS_EMBEDDING_BACKEND` is unset (opt-in default).

**Operator override:** `ATOMIC_AGENTS_EMBEDDING_BACKEND` (provider id), `ATOMIC_AGENTS_EMBEDDING_MODEL` (model id), `ATOMIC_AGENTS_EMBEDDING_DIMENSIONS` (dimension override). All read by `get_default_embedding_backend()`.

**Closes:** the embedding abstraction cliff — both `PgvectorMemoryBackend` (#258 PR 2) and `PgvectorCorpusBackend` (#258 PR 3) share a single injected backend without duplicating provider logic. LOCKED spec/46 carries 9 normative MUSTs (MUST 1 input validation; MUST 2 side-effect-free construction; MUST 3 capability honesty; MUST 4 embed 4-case MUST-NOT-RAISE; MUST 5 URL/secret redaction; MUST 6 storage/key isolation; MUST 7 snapshot/vector determinism; MUST 8 backend_id stability + close() idempotency; MUST 9 len(out)==len(in) conformance invariant).

---

## ConversationBackend (#535, LOCKED spec/47, the twentieth)

Filed as [#535](https://github.com/dep0we/atomic-agents-stack/issues/535). Shipped in PR 1 (Protocol + filesystem impl + full agent.call() wiring) and PR 2 / LOCK PR (model-aware token-budget derivation, check_conversation_backend doctor check, #553 write-back reorder fix).
...
- Guard (2): **two-part check** — the load-bearing identity guard for cross-principal isolation:
  - **(2a) resolved-basename comparison:** `principal_dir.resolve().name == principal.identifier`. A symlink `conversations/bob -> conversations/alice` resolves to `alice`, which differs from `bob`, so the cross-principal read raises `ConversationAccessDenied`.
  - **(2b) inode-identity on-disk name check:** `_ondisk_principal_name(conv_root, identifier)` finds the REAL on-disk directory entry by inode identity (`st_ino`, `st_dev`) and raises `ConversationAccessDenied` when the on-disk name differs in bytes from the requested identifier. Defends the **case-insensitive (macOS APFS) / NFC-NFD aliasing** attack class: `Path.resolve()` returns the caller's spelling, not the on-disk name, so without (2b) principal `alice` could read `Alice` on a case-folding filesystem. Hardlinks are NOT defended (documented not-defended class).
  Stripping EITHER part of Guard (2) causes its strip-RED negative control to fail.
...
**Token-budget window (MUST 8):** oldest-first eviction; `(len(role) + len(content)) // 4 + 1` character-to-token approximation. One-turn floor: if the newest turn alone exceeds `budget_tokens`, returns `[]`. Agent.call() derives budget per-call as `max(1000, model_context_limit - system_prompt_tokens_est - max_output_tokens - 2000)` where `model_context_limit = LLMCapabilities.max_input_tokens`, `system_prompt_tokens_est = len(system_prompt) // 4`. Fails soft to 8000 on any capability-lookup error (UnknownModelError etc.).

**Agent.call() wiring:**
- Three-channel backend selection: constructor kwarg → `ATOMIC_AGENTS_CONVERSATION_BACKEND` env → model.md `## Conversation Backend` field (LOCKED at spec/47). All channels resolve to `None` = single-shot (rule #14, backward-compatible by default).
...
## PrincipalBackend (#556, LOCKED spec/48, the twenty-first)

Filed as [#556](https://github.com/dep0we/atomic-agents-stack/issues/556). Shipped in PR 1 + LOCK PR.

**Reference implementations:** `LocalPrincipalBackend` (home-user zero-config default, `backend_id="local"`, always returns `LOCAL_PRINCIPAL`, `is_local_only=True`) and `StaticClaimsPrincipalBackend` (serve-layer multi-user impl, `backend_id="static_claims"`, maps `{provider, sub}` claims to a 64-char sha256 hex identifier). Both in `atomic_agents/principal/`.

**Protocol surface (spec/48 12-MUST Implementer Contract):** `backend_id` @property; `derive_principal(verified_claims: Mapping) → Principal`; `capabilities() → PrincipalCapabilities`. No write path. WritePolicy NOT applicable. The contract is the 8-MUST PersonaBackend base + four named security-boundary MUSTs: fail-closed-at-the-gate (MUST 10), never-trust-raw-header/never-re-verify (MUST 2), storage-key stability + non-reassignability (MUST 11), and is_verified honesty (MUST 12).

**Key design constraint:** PrincipalBackend is an identity-derivation Protocol, NOT a state store. It derives a `Principal` from already-verified claims (the perimeter — IAP, OIDC middleware — has already verified the token). Construction is always side-effect-free; no filesystem I/O.

**Storage-key encoding (spec/48 MUST 7 + MUST 11):** `sha256(f'{provider.strip(_ASCII_WS)}\x00{sub.strip(_ASCII_WS)}'.encode('utf-8')).hexdigest()` — provider/sub stripped of surrounding **ASCII** whitespace ONLY (`_ASCII_WS = " \t\n\r\f\v"`, NOT `str.strip()`'s Unicode set, so a trailing space cannot split a namespace while two subjects differing only by a leading Unicode space — NBSP `\xa0`, ideographic space — do NOT collide; the Unicode-strip was a security fix closing a MUST 11 non-reassignability hole); the NUL-byte separator prevents prefix-collision attacks (without it, `('google','user')` and `('goo','gleuser')` produce the same concatenated string and alias onto the same conversation directory). The mapping is STABLE (same pair → same identifier across restarts) and NON-REASSIGNABLE (distinct pairs never collide) — MUST 11.

**HARD-REFUSE gate in agent.call():** placed **BEFORE the idempotency dedup lookup** (Phase-1 `lookup()` COMPLETED short-circuit), BEFORE lock acquisition, BEFORE cost gate (MUST 10). Gate condition: `if conversation_id is not None and not principal.is_verified: raise UnverifiedPrincipalConversationAccess(...)`. Gate keys on `is_verified` ONLY — never on object identity with `LOCAL_PRINCIPAL`, never on `derivation_source`. The BEFORE-dedup placement is security-load-bearing: an unverified caller supplying BOTH a `conversation_id` AND a guessed/replayed `idempotency_key` mapping to a COMPLETED record is REFUSED, not served the victim's cached `result_ref` + `replayed_run_id` (cross-principal cached-run replay closed).

**Serve HYBRID flow (fail-closed opt-in):** the raw, client-settable identity header is NOT trusted as a verified claim by default (CWE-290). `ServeConfig.identity_is_perimeter_verified` (default False; serve.md `## Identity Is Perimeter Verified` or `ATOMIC_AGENTS_SERVE_IDENTITY_PERIMETER_VERIFIED`), ANDed with a non-loopback bind, gates the translation. `serve/_app.py` builds `verified_claims` ONLY when that resolved trust is on — and uses the **FULL untruncated `raw_identity` header as the authz `sub`**, NOT the 512-char-truncated `caller_identity` (a security fix: reusing the truncated audit value as the authz key collided two subjects sharing the first 512 chars onto one storage key; `caller_identity` stays capped at 512 for the AUDIT log only). `serve/_runner.py` has three cases — (1) `caller_identity` None **AND perimeter-trust OFF** → `LOCAL_PRINCIPAL` (the `not identity_perimeter_verified` conjunct is a security fix: under perimeter-trust a header-less request must NOT collapse to the shared verified `local` namespace — it falls through to fail-closed); (2) present-but-untrusted, OR perimeter-trust ON but header absent → a directly-constructed `Principal(is_verified=False)` (NOT `LOCAL_PRINCIPAL`, so the posture is backend-independent) → HARD-REFUSE on conversation_id; (3) present-and-trusted → `agent.principal_backend.derive_principal(verified_claims)`, UNLESS the registered backend is `is_local_only` (cross-tenant collapse guard, a security fix: perimeter-trust ON + the default `LocalPrincipalBackend` left registered would IGNORE the claims and return `LOCAL_PRINCIPAL` for every distinct caller, mixing tenants — the runner instead mints `Principal(is_verified=False, derivation_source='serve_local_backend_misconfig')` so the gate fires). Operators MUST configure a STABLE subject header (e.g. IAP `X-Goog-Authenticated-User-ID`) not the rotating JWT assertion (MUST 11). `UnverifiedPrincipalConversationAccess` → HTTP 401.

**spec/40 Export Exemption:** `PrincipalCapabilities` carries NO `supports_canonical_export` field. PrincipalBackend is identity-derivation, not state storage.

**Exception hierarchy:** `PrincipalBackendError(AtomicAgentsError)` (I/O fault — not raised by LocalPrincipalBackend or StaticClaimsPrincipalBackend) + `UnverifiedPrincipalConversationAccess(AtomicAgentsError)` (HARD-REFUSE gate; carries `conversation_id` + `principal_id`; serve → HTTP 401). Both in `atomic_agents/exceptions.py`.

**Operator override:** `ATOMIC_AGENTS_PRINCIPAL_BACKEND` env var. Absent/empty → `LocalPrincipalBackend()`. Known id → that backend. Unknown id → **raises `BackendNotRegistered` LOUDLY** (does NOT silently degrade — a misconfigured org deployment must fail fast; differs from ConversationBackend's soft-degrade). Constructor kwarg wins over env var.

**Doctor:** `check_principal_backend()` in `atomic_agents/doctor.py`, WIRED into `run_doctor()` (appended to the backend-checks list alongside `check_idempotency_backend()`, so it runs in a real `atomic-agents doctor` invocation — not dead code; result name `principal-backend`) — construction probe + positive probe (derive with valid claims) + fail-closed NEGATIVE probe (feed `{}` empty claims, assert `is_verified=False`; skipped for `is_local_only=True` backends — LocalPrincipalBackend returns `LOCAL_PRINCIPAL` with `is_verified=True` by design; a naive negative probe would false-FAIL it) + capability honesty check. FAILs on an unknown `ATOMIC_AGENTS_PRINCIPAL_BACKEND`. Credential-echo-safe: `safe_backend_id = _redact_for_error_message(raw_backend_id)` at all message/detail echo sites; `get_default_principal_backend()` also redacts before constructing `BackendNotRegistered` so `str(e)` never leaks a DSN-shaped env value into doctor output.

**Security fixes (Claude adversarial + cross-family Codex review, each strip-RED negative-control-tested):** (1) cross-tenant collapse guard (`serve/_runner.py` case 3 — is_local_only backend under perimeter-trust → fail-closed `serve_local_backend_misconfig` Principal); (2) storage-key ASCII-whitespace-only strip (`static_claims.py` `_ASCII_WS`, not Unicode — MUST 11 non-reassignability); (3) full untruncated `raw_identity` as the authz `sub` (`serve/_app.py`, not the 512-char audit copy); (4) header-absent-under-perimeter → unverified not `LOCAL_PRINCIPAL` (`serve/_runner.py` case 1 guard is `caller_identity is None AND not identity_perimeter_verified`).

**Test layout:** `tests/test_principal_protocol_conformance.py` (79 collected, parametrized over [LocalPrincipalBackend, StaticClaimsPrincipalBackend]; incl. the ASCII-whitespace-only strip + Unicode-no-collapse + _derive_storage_key parity tests + MUST 2/3/9 object-identity assertions + the derive_storage_key privatization-lock added at LOCK) + `tests/test_principal_local_impl.py` (19 collected — local-impl-specific + registry + env-var dispatch) + `tests/test_principal_call_wiring.py` (28 collected — agent.call() integration, incl. the persisted security-refusal audit record, the `serve_local_backend_misconfig` cross-tenant refusal, the storage-key non-reassignability strip, and the header-absent-under-perimeter fail-closed test, each with a per-invocation negative control). 126 collected in the three principal files + 10 principal-related tests in sibling files (3 test_doctor — 1 added at LOCK, the DSN-redaction regression test; 4 test_serve_app + 3 test_serve_config — pre-existing PR1 coverage, unchanged at LOCK) = 136 principal-related tests; of those, the LOCK ceremony added 7 net new (6 conformance: MUST 2/3/9 + derive_storage_key privatization-lock; 1 doctor DSN-redaction). `derive_storage_key` made private (_derive_storage_key, underscore-only at LOCK).

**Closes:** the identity-derivation cliff — `agent.call()` now carries a typed, verified caller identity, conversation namespaces are gated by `is_verified`, and unverified callers requesting conversation access are refused before any LLM spend.
