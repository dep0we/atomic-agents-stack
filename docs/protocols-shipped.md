...
## PrincipalBackend (#556, DRAFT spec/48, the twenty-first)

Filed as [#556](https://github.com/dep0we/atomic-agents-stack/issues/556). Shipped in PR 1 of a multi-PR arc.
...
**Test layout:** `tests/test_principal_protocol_conformance.py` (73 collected, parametrized over [LocalPrincipalBackend, StaticClaimsPrincipalBackend]; incl. the ASCII-whitespace-only strip + Unicode-no-collapse + derive_storage_key parity tests) + `tests/test_principal_local_impl.py` (19 collected — local-impl-specific + registry + env-var dispatch) + `tests/test_principal_call_wiring.py` (28 collected — agent.call() integration, incl. the persisted security-refusal audit record, the `serve_local_backend_misconfig` cross-tenant refusal, the storage-key non-reassignability strip, and the header-absent-under-perimeter fail-closed test, each with a per-invocation negative control). 120 collected in the three principal files + 9 net-new in modified files (2 test_doctor, 4 test_serve_app incl. full-identity-not-truncated `sub`, 3 test_serve_config) = 129 new collected tests.
