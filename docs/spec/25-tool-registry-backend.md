...
4. **Descriptor round-trip MUST be lossless for the dispatch fields, with backend-shape-appropriate fidelity.** The contract has two tiers:
   ...
   **Conformance-test pinning status (v1 lock):** the parametrized conformance suite currently pins round-trip on `name` (via `list_tools()` + `load_tool()`), `classification` (via `test_tool_ref_classification_round_trip`), and `version` (currently `None` on both reference backends). It does **not yet** pin Tier B `input_schema` / `description` / `handler` callable round-trip on structured-storage backends — both reference backends preserve them in practice, but a future Tier B backend (PyPI, SaaS-database) could silently drop them and pass green. Closing that gap is tracked in [#207](https://github.com/dep0we/atomic-agents-stack/issues/207); the MUST is normative regardless, and future Tier B adapters are responsible for verifying it via their own regression tests until #207 lands.
