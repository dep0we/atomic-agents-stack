...
- `tests/test_tool_registry_protocol_conformance.py` — **37 tests parametrized via `BACKEND_FACTORIES`** (only filesystem in PR 1; PR 3 adds SQLite for ×2 = ~74 parametrized invocations + capability-gated skips for install/uninstall on filesystem). Tests cover: Protocol surface, `backend_id` stability, `capabilities()` shape, `list_tools` empty/populated/lexicographic, `load_tool` returns `ToolDefinition` with callable handler, `load_tool` raises `ToolNotInRegistry` for missing, `validate` static (does NOT execute handler), `validate` reports import + descriptor errors, capability-gated install/uninstall raise NotImplementedError on filesystem, classification round-trip on `ToolRef`, version field round-trip (currently None on filesystem), descriptor frontmatter validation, descriptor handler-module-not-found, descriptor handler-not-callable, path-traversal on tool name refused, source attribution populated, integration with existing `ToolNameCollision`.
- `tests/test_tool_registry_filesystem_backend.py` — **23 filesystem-specific tests**: on-disk descriptor parsing, hidden-file exclusion, `tools/` absent → empty list, `tools/__init__.py` ignored, `tools/_helper.py` ignored, handler module re-import semantics, `validate()` warnings vs errors, registry resolution via `register_tool_registry_backend`, `get_default_tool_registry_backend` env-var dispatch + invalid backend_id + credential redaction + truncation, source field shows descriptor's filesystem path.
...
Total at PR 1 close: **~60 net tests**; full suite goes 1750 → ~1810.
Total at arc close (PR 4 lock): **~104 conformance + ~50 backend-specific + ~17 wiring = ~170 tests** verifying the Protocol contract across both reference backends.
