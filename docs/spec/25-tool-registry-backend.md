...
**Why:** the filesystem reference impl walks `tools/<name>.md` for metadata (cheap, just markdown parsing) and ONLY runs `importlib.util.spec_from_file_location` when `load_tool(name)` is called. **The lazy/eager distinction is at the BACKEND layer — not the wiring layer.** PR 2 of #64 wires `AtomicAgent.__init__` to call BOTH `list_tools()` AND `load_tool(name)` for every backend ref at construction time, so handler-module imports DO fire on every agent construction (not lazily on first dispatch). Operators with side-effecting top-level code in `<agent>/tools/<name>.py` see the side effect at agent construction. This is acceptable in the filesystem-trust-boundary model (operators own `<agent>/tools/`); a future capability flag `lazy_handler_import: bool` MAY ship to support fully-lazy dispatch for backends serving untrusted catalogs (PyPI / SaaS / HTTP).
...
- **TOCTOU between `list_tools()` and `load_tool(name)` + redundant-parse perf gap.** PR 2's wiring loop (`for ref in backend.list_tools(): td = backend.load_tool(ref.name); registry.register(td)`) re-reads each descriptor from disk inside `load_tool` — if an operator (or concurrent process) swaps the descriptor between the two calls, the in-memory registry holds metadata from version A while dispatch routes through handler code from version B. At N=50 tools the redundant parse wastes ~3 ms per agent construction (~60 μs per tool, measured by Step 9.1 perf specialist on PR 2). PR 2 did NOT implement the fix — re-targeted to a successor follow-up issue. Fix shape: internal `_iter_parsed()` helper returning `(ref, parsed_descriptor)` pairs + a `load_tool(name, parsed=...)` overload threaded through the wiring loop. Tracked in `[backend] FilesystemToolRegistryBackend list_tools/load_tool parsed-passthrough` (successor issue to be filed).
...
4. **Raw-text round-trip MUST be byte-for-byte preserving for descriptor content.** Backends serving operator-supplied tool descriptors MUST persist the raw descriptor body (frontmatter + markdown comments) and reconstruct via parsing on each `load_tool` — never store ONLY the parsed structured fields. The reference `FilesystemToolRegistryBackend` reads `<agent>/tools/<name>.md` verbatim from disk on each `load_tool`; the reference `SQLiteToolRegistryBackend` serializes the descriptor JSON shape (`name`, `description`, `classification`, `input_schema`) and re-derives `ToolDefinition` on `load_tool` — both round-trip via the existing `_tools.parse_tool_descriptor` path. The same lesson from spec/24 Decision 1 / MUST #4 applies: parsers are lossy (operator notes after the markdown body, comments inside YAML, etc.) and a structured-only persistence shape would silently drop them. `ToolRef` is **append-only** on the canonical type — removing fields is a breaking change; adding fields is a minor-version bump with `None` / empty defaults. Backends MUST round-trip ALL `ToolRef` fields (including `version` and `source`, which the filesystem reference always sets to `None` / a path string respectively) so JSON-shape forward compat holds across future field additions.

5. **Schema initialization MUST be idempotent across processes AND `PRAGMA busy_timeout` MUST precede `PRAGMA journal_mode=WAL` for SQLite-shape backends.** Multi-process operators may have N replicas all opening a fresh backend simultaneously (Cloud Run / Kubernetes / multi-replica shapes). The reference `SQLiteToolRegistryBackend._ensure_schema` ships TWO lessons learned across the protocol-pattern arcs: (a) the `INSERT OR IGNORE INTO meta (key, value) VALUES ('schema_version', '1')` cold-start race fix from #61 PR 3 SQLiteLogBackend Step 11 P0 #2 (multi-process schema init mustn't deadlock or raise UNIQUE-constraint errors), AND (b) the **new lesson from #64 PR 3 Step 11**: `PRAGMA busy_timeout=5000` MUST be set BEFORE `PRAGMA journal_mode=WAL`. The WAL-transition lock contention manifests as `OperationalError: database is locked` (REPRODUCED 3/5 pre-fix in PR 3 Step 11) when concurrent processes simultaneously open a fresh db and race the journal-mode transition. The busy_timeout makes the second process wait instead of failing — same shape as the pre-existing `test_log_sqlite_backend.py::test_concurrent_appends_from_threads` flake (a one-line follow-up PR queued in spec/22 §"Known gaps"). Future Postgres / MySQL / non-SQLite backends MUST identify the equivalent cold-start-race primitive (transactional schema migrations, advisory locks during DDL) and document it in the backend's module docstring.
...
- `tests/test_tool_registry_protocol_conformance.py` — **~37 conformance test functions parametrized via `BACKEND_FACTORIES`** running across filesystem + SQLite for ~74 parametrized invocations (with capability-gated skips: `supports_install` / `supports_skills_catalog` / `supports_versioning` tests skip on the backend that declares False; `_capability_parity_install` is rewritten as a real install + verify-in-`list_tools` round-trip after PR 3 Step 11's "tautology" CRITICAL finding). Tests cover: Protocol surface, `backend_id` stability, `capabilities()` shape, `list_tools` empty/populated/lexicographic, `load_tool` returns `ToolDefinition` with callable handler, `load_tool` raises `ToolNotInRegistry` for missing, `validate` static (does NOT execute handler), `validate` reports import + descriptor errors, capability parity for `supports_install` / `supports_uninstall` / `supports_versioning`, classification round-trip on `ToolRef`, version field round-trip (None on both reference backends; field shape is forward-compat), descriptor frontmatter validation, descriptor handler-module-not-found, descriptor handler-not-callable refused at install time (PR 3 Step 11 testing CRITICAL fix), path-traversal on tool name refused at the API boundary, source attribution populated, integration with existing `ToolNameCollision`. The conformance helper `make_tool_in_backend(backend, ...)` uses each backend's Protocol surface for setup (filesystem writes `.md`+`.py` to disk; SQLite calls `install()`) so backend-specific factory differences don't leak into the conformance tests — plan-subagent Risk J from PR 3.
