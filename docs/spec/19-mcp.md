...
> Status: **implemented** (PR feat/mcp-support)
...
- `env`: `KEY=$VAR_NAME` pairs (one per line or comma-separated). `$VAR_NAME` is
  ...
  raises `MCPServerConnectFailed` at agent init.
...
agent.call() called
    ↓
[if mcp_servers non-empty AND pool is None]
MCPClientPool.connect_all()
    → for each server: asyncio.run(_async_connect_and_list)
        → stdio_client(params) spawns subprocess
...
MCPClientPool.discover_tools()
    ...
    → registers ToolDefinition in agent's ToolRegistry
    ↓
...
**Path-traversal best-effort:** When parsing `mcp.md`, `parse_mcp_md_text`
checks path-shaped args (starting with `/`, `~`, or containing `/`) against the
agent's declared `read_paths` from `tools.md`. Args that resolve outside all
declared read paths raise `PathTraversalError`. This is best-effort — the
framework cannot know what every MCP server treats as a path, but the obvious
path-shaped cases are caught.
