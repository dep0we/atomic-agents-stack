...
|---|---|---|---|
| Spend-gate prior-cost sums | `judge/mandate_check.py` `_sum_prior_token_cost` / `_sum_prior_external_cost` | **In the same function** (the narrow `except _NARROW_READ_FAILURE` branch) | **fail-CLOSED when the corresponding EFFECTIVE cap is active** — mandate-declared OR Policy-composed, `MIN(mandate, Policy)` (cap-gated, #512) → BLOCK `mandate_cost_unreadable`; degrades to `$0.00` only when no cap exists at either layer (the sum gates nothing) |
| Outstanding-reservation sums | `judge/mandate_reservations.py` `compute_outstanding` (**UNWRAPPED**) | **At the callers** (`mandate_check.py` step-7/step-8) — a typed `except LogBackendReadError` (#497) precedes the broad `except Exception` | **fail-CLOSED on `LogBackendReadError`** (#497 interim guard → BLOCK `mandate_{token,external}_reservations_unreadable`); the prior-cost-sum narrow family is **cap-gated** (#506 flip → #512 gate) — see row 1 |
| Startup orphan-recovery scan | `mandate/filesystem.py` `_scan_orphan_reservations` (**UNWRAPPED**) | **At the caller** (`agent.py` around `recover_orphan_reservations`) | degrade-and-skip recovery |

The new `LogBackendReadError` IS caught for every mandate read site. #497 was the first half (the outstanding-reservation read); #506 + #512 completed the second (the prior-cost sums) — so the live mandate spend-gate now fails closed when blind **AND a corresponding cap (mandate-declared OR Policy-composed) is in effect**, degrading to `$0.00` when no cap exists. The **outstanding-reservation** read (`compute_outstanding` at the step-7/step-8 callers) gets a typed `except LogBackendReadError` that fail-CLOSES (immediate BLOCK `mandate_{token,external}_reservations_unreadable`), mirroring the layered catch added to `_costs._sum_via_backend`. The **prior-cost sums** (`_sum_prior_token_cost` / `_sum_prior_external_cost`) catch the narrow read-failure family (`LogBackendReadError`, `OSError`, `sqlite3.DatabaseError`) and — #506 flipped them from the prior `$0`-prior-spend fail-OPEN to a fail-CLOSED `BLOCK mandate_cost_unreadable`; #512 then **cap-gated** that flip — fail closed only when the corresponding **effective** cap is active (mandate-declared OR Policy-composed via `MIN(mandate, Policy)`), degrading to `$0.00` when no cap exists at either layer (the sum gates nothing). The full normative statement of this posture lives in **spec/29 §"Blind-read fail-closed posture"** (the LOCKED home for the mandate spend-gate's read behavior); this section is the cost-read-surface companion. A code defect (`KeyError`/`TypeError`/`AttributeError`) is deliberately NOT caught as a read failure — it propagates so the audit record names a bug rather than mislabelling it a blind-read event. **Granularity note (filesystem default):** on the pre-#497 baseline a non-ENOENT per-file `OSError` during a mandate read was silently skipped (`except OSError: continue`), so the read completed with *partial* data (a small undercount); after #497/#506 that same `EIO`/`EACCES` day file raises `LogBackendReadError` and — at every mandate read site whose cap is active — BLOCKs fail-closed (no undercount). The recovery scan's per-file granularity changes from partial-recovery to whole-scan-skip-and-log at the orchestration layer (still no crash, still degrade-and-skip). Deciding the recovery-scan raise-vs-skip posture (the live-gate-vs-recovery-scan asymmetry) is tracked as [#511](https://github.com/dep0we/atomic-agents-stack/issues/511).
