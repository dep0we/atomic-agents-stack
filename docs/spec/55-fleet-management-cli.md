...
- `<rec-id>` — an opaque identifier for one currently-computable `savings_cost` recommendation (see "Rec-identity" below). Exactly how an operator obtains a `<rec-id>` to pass here — copied from the console's rendered card, or resolved from `--agent <name>` at the CLI — is an open fork (below); the identifier itself is a recomputed hash, never a stored row.
- `--dry-run` — preview only (S2 step 2): shows the resolved `(agent, current_model → candidate_model)` swap AND the outcome of the apply-time re-validation (below), without writing. No write happens whether re-validation would pass or fail.
- `--yes` — apply without the interactive confirm (S2 step 3). Identical semantics to `govern`/`set-model`.
- `--json` — structured output (S3), including a structured reason on every refusal path introduced by this verb (`rec_no_longer_valid`, `rec_guard_failed`, `rec_kind_not_applicable`), in addition to the full `set-model` M9 refusal vocabulary it composes with unchanged.
- `--agents-root` — fleet-scoped agent resolution, consistent with every other verb.

`apply-rec` has no `--show`/`--list-snapshots`/`--restore` of its own — it creates no new restorable state (see M1 composition below). Inspecting or rolling back the resulting `model.md` change uses `set-model --show` / `set-model --list-snapshots` / `set-model --restore` directly, against the SAME snapshot tree `apply-rec`'s delegated write populates.

### Behavior (the S2 routine, concretely)

1. **Resolve + validate.** Re-run `recommend_fleet(agents_root)` (the pure-compute recompute — spec/54; recommendations are NEVER persisted, so this call is unconditional and always fresh) to obtain the CURRENT set of recommendations across the fleet. Compute the rec-identity hash (below) for every returned `Recommendation` and look for one matching `<rec-id>`. No match → refuse `rec_no_longer_valid` (see "Rec-identity"). A match whose `kind != "savings_cost"` → refuse `rec_kind_not_applicable` (see "Applicable rec kinds"). Otherwise, re-run `_eval_headroom(...).passed` re-validation on the matched recommendation's safety data (see "Apply-time re-validation") — a `False` result refuses `rec_guard_failed`. Only past all three gates does `apply-rec` delegate into `set-model`'s OWN step-1 resolve + M9 composition chain (PRICING/backend-resolution/policy-consult) against the matched `candidate_model` — a recommendation that named a model since deprecated from `_costs.PRICING`, or now claimed by more than one `LLMBackend`, is refused by `set-model`'s existing gates, unchanged, with `set-model`'s existing `error_type`s. All of this — the rec-match, the guard re-check, AND the delegated `set-model` validation — happens BEFORE step 2, per M4.
2. **Preview.** Render the SAME before→after `set-model` would render for `--model <candidate_model>` (the `## Default model` value line, everything else unchanged), plus the recommendation's own rationale and projected `$/mo` savings so the operator sees WHY the swap is recommended, not only WHAT changes. Nothing is written.
...
5. **Audit.** See "Audit" below — an open fork on the exact primitive, but either resolution carries a `RunRecord` identifying that this write was rec-driven, not operator-typed.

### Rec-identity — deterministic recomputed hash (MAINTAINER-RULED, 2026-07-06)

A `Recommendation` (`atomic_agents/advisor/recommend.py`) has no persisted identifier and no persisted row to look one up in — `recommend_fleet()` is a pure-compute recompute over 30-day JSONL windows, called fresh on every console render and every `apply-rec` invocation (spec/54; ZERO new LLM spend, ZERO state store). A `<rec-id>` is therefore, by ruling, a **deterministic hash of the recommendation's identifying fields** — `(agent, kind, candidate_model)`, the three `Recommendation` fields (`atomic_agents/advisor/recommend.py`) that together name "this specific swap for this specific agent" — NOT a stored value, NOT a database row, NOT a new sidecar file (composes M1: `apply-rec` MUST NOT introduce a management-state store to persist recommendation identities).

Concretely: the console renders a `<rec-id>` next to each `savings_cost` card by hashing the SAME three fields at render time; `apply-rec` independently recomputes `recommend_fleet()` and hashes every current recommendation's SAME three fields the SAME way; a match means "a recommendation for this exact agent/kind/candidate-model swap still holds today," and a non-match means the console's rendered rec has gone stale — the agent's usage repriced, the candidate model changed, or the underlying data moved enough that `recommend_fleet()` no longer produces that exact triple. **`rec_no_longer_valid` is a first-class, expected refusal, not an error condition** — a stale console card MUST NOT silently apply a swap the current data no longer recommends; this is the correct behavior for a system whose recommendations are recomputed, not committed. The exact hash recipe (algorithm, field encoding/delimiter, and whether the console/CLI show a full hash or a short prefix) is an open fork (below) — what is settled is the three input fields and that the value is ALWAYS recomputed, never looked up in a store.

### Apply-time re-validation — the no-quality-cost guard, again

The console rendered this recommendation against a no-quality-cost guard that was true AT RENDER TIME (spec/54 MUST 2–6, the composite conjunctive `EvalHeadroom` gate — `weighted_score_margin`, `pass_rate_margin`, `hard_fails`, `sample_n`, all clearing their floors). Time passes between a console render and an operator running `apply-rec`; new evals, hard-fails, or usage may have landed in that window. `apply-rec` MUST NOT trust the render-time guard result — it MUST re-run `_eval_headroom(eval_records, rubric_threshold, rec_config)` (`atomic_agents/advisor/recommend.py`) fresh, over the SAME 30-day window discipline `recommend_fleet()` uses, and require `.passed` before delegating to `set-model`. A rec whose guard passed when the console rendered it but no longer passes at apply time — a new hard-fail landed, the pass-rate margin eroded — is refused (`rec_guard_failed`), distinct from `rec_no_longer_valid` (the rec doesn't match at all) and distinct from any `set-model` M9 refusal (the candidate model itself has a problem). The guard is pure and cheap to re-run by construction (spec/54 — zero LLM spend, no state beyond already-loaded JSONL), which is exactly what makes "just re-run it" the right answer instead of trusting a render-time cache.

This re-validation happens IN ADDITION TO, not instead of, `set-model`'s own M9 composition chain (PRICING/backend-resolution/policy-consult) against the candidate model — the two guards check different things: `_eval_headroom` asks "is this agent's quality still safe to downgrade," `set-model`'s M9 asks "is this candidate model still a valid, priced, resolvable choice." Both MUST hold. Neither is skipped because the other passed.

### The delegation seam

`apply-rec` MUST NOT duplicate `set-model`'s surgical `model.md` writer, its lease/snapshot/restore machinery, or its M9 composition chain — reimplementing any of those would violate the "compose, never bypass" discipline (M9) the same way a hand-rolled model-write bypassing `set-model` entirely would. `apply-rec` resolves `<rec-id>` to a concrete `(agent, candidate_model)` pair and THEN drives `set-model`'s existing write routine with that pair, exactly as if an operator had typed `manage set-model <agent> --model <candidate_model>` themselves — the snapshot lands in the SAME `.config-snapshots/set-model/` tree (no separate `apply-rec` snapshot namespace), and `set-model --restore`/`--show`/`--list-snapshots` work identically on a rec-applied write as on a hand-typed one.

The EXACT mechanism for that seam is an open fork (below): `run_set_model(args: Any, agents_root: Path) -> int` (`atomic_agents/manage/set_model.py:596`) reads its target agent, model id, and flags off `args` via `getattr(args, ..., default)` for every field except `agent_id: str = args.agent` (a hard, non-defaulted attribute access) — so a caller can drive it either by synthesizing a minimal namespace object carrying at least `.agent` and `.model` (plus whichever of `.json`/`.dry_run`/`.yes` it wants non-default), or by factoring a lower-level shared entry point that both the `set-model` CLI dispatch and `apply-rec` call directly, bypassing the `args`-shaped surface entirely. Which of these `apply-rec` should do is not decided by this section.

### Audit

Every applied `apply-rec` write MUST be traceable back to "this was a rec-driven apply, not an operator hand-typing `--model`" in the audit trail (principle #5) — an operator or copilot reviewing the fleet audit stream needs to distinguish the two. The exact shape is an open fork (below): a NEW dedicated primitive (`PRIMITIVE_MANAGE_APPLY_REC`, alongside `PRIMITIVE_MANAGE_GOVERN`/`PRIMITIVE_MANAGE_SET_MODEL`/`PRIMITIVE_MANAGE_RESTORE` in `atomic_agents/logs/types.py`), OR the existing `PRIMITIVE_MANAGE_SET_MODEL` primitive with an added `applied_from_rec` marker in `extra{}` (carrying the matched rec-id and `kind`) so a single `set_model`-family query still surfaces rec-driven writes as a filtered subset. Either resolution reuses the M8-pinned `RunRecord` SHAPE unchanged — this section does not re-pin it, matching the precedent `set-model`'s own "Audit primitive" subsection sets ("the primitive id is explicitly NOT one of the pinned-identical parts").

### Applicable rec kinds

PR1 of `apply-rec` applies **`savings_cost` recommendations only** — the one kind with a mechanical, unambiguous "apply" action (a model swap). `quality_report` (surface an eval/tuning-report signal) and `governance` (governance.md is absent/invalid) recommendations are, and remain, advisory-only — there is no mechanical "apply" for "go read this tuning report" or "go write governance.md by hand" (the latter has its own editor, `manage govern`, which an operator drives directly; `apply-rec` does not chain into `govern`). Calling `apply-rec` with a `<rec-id>` that resolves to a `quality_report` or `governance` recommendation MUST refuse cleanly (`rec_kind_not_applicable`) rather than silently no-op or attempt an unsupported write.

**Forward note (#649 / #644-child-D coordination).** A `savings_cost` recommendation's underlying pricing signal is, today, a straight reprice of 30-day actual usage at a cheaper same-family sibling's PRICING rate (spec/54). The Model Testing & Capability Scorecard design (#644-child-D, spec/55 cross-reference pending) anticipates a SECOND `savings_cost` source — a `measured_scorecard`-backed recommendation, where the candidate model has been validated against a ground-truth test set rather than assumed same-family-safe. `apply-rec`'s rec-identity hash and re-validation gate are designed against TODAY's single source; whether a future `measured_scorecard`-sourced rec needs a distinct re-validation path (e.g. re-checking the scorecard result instead of, or in addition to, `_eval_headroom`) is explicitly NOT decided here — flagged so a future scorecard arc revisits this section rather than silently drifting from it.

### Conformance test outline (apply-rec additions)

Mirrors the shape of the top-level "Conformance test outline" section and `set-model`'s own additions to it; these are the `apply-rec`-specific additions, not a replacement:

- **Rec-match:** a `<rec-id>` recomputed from a currently-valid `savings_cost` recommendation resolves and applies; a `<rec-id>` that matches no current recommendation refuses `rec_no_longer_valid`, non-zero exit, no write (positive + negative control pair).
- **Guard re-validation:** a recommendation whose `_eval_headroom(...).passed` was `True` at a simulated "render time" but is made `False` by injecting a fresh hard-fail before `apply-rec` runs refuses `rec_guard_failed`, distinct from `rec_no_longer_valid` and distinct from any `set-model` M9 refusal — asserted as three separately strip-tested negative controls (per the project's per-invocation negative-control discipline), so a partial re-validation implementation cannot false-green.
...
- **Delegation fidelity:** an applied `apply-rec` write is byte-identical in its `model.md` result, snapshot location, and lease/audit behavior to a hand-typed `set-model --model <candidate_model>` run against the same agent and starting state — asserted by running both paths against identical fixtures and diffing the outcomes.
...
- **Spine inheritance:** `--dry-run`/`--yes`/`--json`/exit-code ladder/manage lease/snapshot-restore all behave identically to a direct `set-model` invocation, verified by the SAME spine test suite parametrized over both entry points where feasible.

### Open forks (arc-discovery #727)

These are the genuinely undecided questions this section deliberately leaves open for the discovery pass — none are settled by this revision:

- **The set-model delegation seam.** Should `apply-rec` synthesize a `set-model`-shaped `args` object (e.g. a minimal namespace carrying `.agent`/`.model`/`.json`/`.dry_run`/`.yes`) and call `run_set_model(synthesized_args, agents_root)` directly, or should a lower-level shared write entry point be factored out of `set_model.py` that both the CLI dispatch and `apply-rec` call, so neither is coupled to an `args`-shaped surface? Prefer the latter if `run_set_model` proves too argv-coupled to synthesize cleanly against.
- **Audit primitive.** A new `PRIMITIVE_MANAGE_APPLY_REC`, or the existing `PRIMITIVE_MANAGE_SET_MODEL` primitive plus an `applied_from_rec` marker in `extra{}`?
- **Rec-id hash exact recipe.** Which fields, in what order and encoding, feed the hash (`agent` + `kind` + `candidate_model` is settled; the exact join/serialization and hash algorithm are not); how is the hash kept stable across future `recommend.py` refactors that might reorder or rename fields; is a short displayed prefix sufficient for the console card and CLI argument, or must the full hash be passed?
- **The re-validation refusal vocabulary.** Are `rec_no_longer_valid` (no current recommendation matches the hash), `rec_guard_failed` (a match exists but `_eval_headroom` no longer passes), `rec_kind_not_applicable` (a match exists but isn't `savings_cost`), and the inherited `set-model` M9 `error_type`s (`unpriced_model`/`unknown_model`/`ambiguous_backend`) sufficiently distinct for a copilot driver to react correctly to each, or should any be merged/split further?
- **Surface.** `apply-rec <rec-id>` as a single opaque positional, or `apply-rec --agent <name> --rec <id>` (or both, with the id disambiguating within the named agent)? Does `--dry-run` show the resolved swap AND the full re-validation result (both guards), or only the swap with re-validation deferred to the confirm step?
...
- **Epic #606** — Agent Fleet Platform (observe → manage → interact); #624 (this foundation), #609 (the `govern` verb), #726 (the `set-model` verb).
...
- **TENSIONS T17** — core↔extension boundary; `manage/` is an extension package and routes PRICING lookups through `atomic_agents.core_api.get_model_rates()`, never `_costs.PRICING` directly.
