...
- The **third verb**, `manage apply-rec <rec-id>` (#727) — the Fleet Console recommendation loop-closer; a composition verb that re-validates a `savings_cost` recommendation against current data and, if it still holds, delegates the write into `set-model`'s own routine. Specified below ("Third verb"); implemented in #727 Unit 2.
...
**PR1 scope (ruled — arc-discovery #726, `pr1-flag-scope`): `--model` ONLY.** `--fallback` (#754) and `--provider` (#755) are grammar-recognized by the CLI parser (their names/semantics are pinned below, matching the CLI-surface-grammar precedent for `govern`) but return a clean, honest `"not yet settable in PR1"` refusal — never an argparse "unrecognized arguments" error, so the flag's parse status never shifts across PRs. The refusal fires BEFORE the registry resolve (a scope refusal independent of whether the named agent exists), mirroring `govern`'s own grammar-pinned-but-deferred flags (`--add`/`--remove`/`--set-json`).
...
- `--model <id>` — the new `## Default model` value. MUST pass the full M9 composition chain (below) before anything is written. The verb's PR1 reason to exist.
- `--fallback <id>` — the new `## Fallback` value, subject to the SAME M9 composition chain as `--model` when it ships. **Deferred to #754 (ruled `fallback-m9-bar`) — grammar-recognized now, not yet settable.** The flag name and semantics are pinned here so the contract never shifts; PR1 returns the deferred-flag refusal for it.
- `--provider <id>` — disambiguates `find_backend_for_model` when a requested model id is claimed by more than one registered `LLMBackend`, writing (or updating) the optional `provider:` line that lives OUTSIDE the `cost_guardrails` yaml block (`atomic_agents/_model.py:105-106`). **Deferred to #755 (ruled `provider-disambiguation-posture`) — grammar-recognized now, not yet settable.** The insert-when-absent `provider:`-line surgical writer ships with #755, not PR1.
- `--show` — print the current resolved model config (`default_model`, `fallback_model`, `provider`) as parsed by `parse_model_md`, PLUS one real signal — whether Policy currently OVERRIDES the written `default_model` (ruled `show-json-shape`; the effective-vs-written distinction, tied to the WARN-AND-WRITE ruling below). Read-only, respects `--json`. Same convention as `govern --show`, extended with the one real signal set-model's M9 makes possible.
...
- `--restore <snapshot-id>` — roll back `model.md` to a prior `set-model` snapshot, through the SAME hoisted spine `govern --restore` uses (M3 sub-MUSTs (a)/(b)): resolves only under the target agent's own `.config-snapshots/set-model/` tree, itself takes a pre-restore snapshot, emits exactly one `PRIMITIVE_MANAGE_RESTORE` record. **Restore does NOT re-run the M9 composition chain** (ruled `fallback-m9-bar` extends the same rule set-model shares with `govern`) — identically to `govern`'s restore (§"Restore verb"), a snapshot is by definition prior content that already passed composition once; re-validating a historical write against TODAY's PRICING/registered backends/`policy.md` would make old snapshots unrestorable the moment a model is deprecated, defeating the point of a rollback path. A restored snapshot whose model id is no longer PRICED surfaces a non-blocking ADVISORY note (never a refusal) so M9 gate (a)'s bypass-by-design on restore is never silently invisible — this advisory checks pricing ONLY (`get_model_rates(...) is None`); there is no matching advisory for gate (b) (backend resolution) today, a documented gap, not an implementation.
- `--dry-run`, `--yes`, `--json`, `--agents-root` — identical semantics to `govern`'s (S2/S3), unchanged.

### Behavior (the S2 routine, concretely)

1. **Resolve + validate.** Load the target through `AgentRegistryBackend` (S1). Unlike `governance.md`, `model.md` is rendered by every `init` template (`atomic_agents/init/templates/*/model.md`) — its absence is not the ordinary "first time an operator configures this" case `govern`'s create-absent path exists for. **model.md-presence IS the `AgentRegistryBackend` discovery predicate (spec/37:314)**, so an agent missing `model.md` never resolves through S1 at all — an operator invoking `set-model` against such an agent sees `error_type='agent_not_found'` at the registry-resolve step, never a `set-model`-specific refusal. `set-model` ALSO carries its own `model_md_absent` check (`error_type='model_md_absent'`) as a direct-API/TOCTOU defense-in-depth safety net — UNREACHABLE through the real end-to-end CLI, reachable only by a direct caller of the internal write-path function or by a race where `model.md` is deleted between S1 resolve and this check — it does not carry `govern`'s create-absent allowance and does not render a stub. If `model.md` IS present but has no `## Default model` heading, `set-model` MUST refuse cleanly (`error_type='default_model_heading_absent'`, ruled `absent-heading-in-present-file`) — "edit model.md directly," and this refusal IS reachable by an operator (an agent can resolve through the registry with a present-but-malformed `model.md`). set-model is a surgical editor, not a scaffolder. Then run the full M9 composition chain against `--model` (PR1 scope — `--fallback` when #754 ships) before anything is previewed — refuse before step 2 on the first failing check, exactly as M4 requires, each gate with its own distinct `error_type` (ruled `refusal-error-taxonomy`).
2. **Preview.** Show the before→after of ONLY the targeted heading value line (`## Default model`) — the `cost_guardrails` block, the `provider:` line, and every prose paragraph render as unchanged. When Policy currently overrides the value being written, the WARN (below) is shown here too, BEFORE the operator confirms — not only in the post-write success payload.
...
`govern`'s M9 composition set is empty by design (no governance field has a runtime-rejection counterpart). `set-model` is the first verb where M9 does real work: a `--model` value MUST be refused at write time — before preview, per M4 — unless checks (a) and (b) below hold. Each check reuses an existing pure function; `set-model` MUST NOT reimplement pricing, backend resolution, or policy evaluation:

- **(a) Priced (ruled `unpriced-model-posture` — hard refusal, always, no `--force`).** The value MUST be a key in `atomic_agents._costs.PRICING` (checked through the core↔extension boundary accessor, `atomic_agents.core_api.get_model_rates(model_id) is not None` — TENSIONS T17; `manage/` is an extension package). An unpriced model is not a cosmetic gap — a scaffolded agent whose default model falls outside `PRICING` bills through fallback pricing, a real, silent over-bill hazard (the same hazard the project's "bump a model" memory lesson names for registering new models). `error_type='unpriced_model'`.
- **(b) Resolvable to exactly one backend (ruled `provider-disambiguation-posture`).** The value MUST resolve to exactly one registered `LLMBackend` via `atomic_agents.llm.find_backend_for_model(model_id, preferred_provider=None)` — the full resolution chain fires in PR1 (only the *resolution* of an ambiguity via `--provider` is deferred). Zero matches raises `UnknownModelError`, refused as `error_type='unknown_model'`. More than one match raises `AmbiguousBackendError`, refused as `error_type='ambiguous_backend'` with a message that points at the DEFERRED capability (#755) — it MUST NOT tell the operator to "re-run with --provider," a flag not yet settable in PR1. This message is DELIBERATELY NOT `str(AmbiguousBackendError(...))` — that exception's own text hardcodes exactly the `--provider` guidance this ruling forbids.

**(c) Policy consult — CONSULT-ONLY in PR1, not a refusal gate (escalated to the maintainer, not silently decided — see "Caps-compose: an open implementation question" below).** `PolicyBackend.get_effective_caps(agent_name)` is CONSULTED (fetched, available for a future gate) but produces NO refusal in PR1 — `CostCaps` carries only `daily_usd`/`monthly_usd` aggregate spend totals, and there is no sanctioned `(model_id, CostCaps) -> forbid/allow` formula anywhere in the codebase today. Inventing one during this build would be exactly the unruled heuristic the composition-gate discipline forbids ("MUST NOT reimplement... policy evaluation"). This is a real, currently-open implementation question — not settled by this spec revision — tracked for an explicit maintainer ruling before a real caps-forbid check is coded.

**Policy model-override interaction — WARN-AND-WRITE, not a refusal (ruled `policy-override-interaction`, verified against `agent.py:3696` — Policy's `get_effective_model()` wins over `model.md` at the `agent.call()` consumption site).** `set-model` writes the operator's `--model` value to `model.md` UNCONDITIONALLY — `model.md` is the base layer; Policy is a deliberate override on top, and a management write's job is to keep the base layer accurate, not to enforce a runtime precedence rule about a DIFFERENT layer. When `PolicyBackend.get_effective_model(agent_name)` returns a non-`None` value that DIFFERS from `--model`, the applied write emits a prominent, source-named WARNING ("Policy currently overrides this agent's model with `<Y>`; your model.md change will not take effect at runtime until that Policy override is removed") — surfaced at BOTH the preview stage (before confirm) and again from a BEST-EFFORT post-write recompute (so a Policy change introduced during an unbounded confirm-wait is far less likely to make the persisted audit lie about the override state at the moment of the write — narrowed, not eliminated: the recompute runs after the manage lease is released, and `policy.md` carries no lock of its own, so this is not a hard atomicity guarantee). This is a WARN, not a refusal — exit `0`, write applied, `error_type` is never populated for this case. The post-write recompute value is recorded in the audit record's `policy_override_at_write` field (see "Audit primitive").

PolicyBackend construction or read failure is NOT uniformly fail-closed across the two consults above — the two have opposite postures, matching whether a write has landed yet. The PRE-write consult (gate (c), before anything is written) is FAIL-CLOSED: `error_type='policy_backend_unavailable'`, mirroring `ManageLockUnavailableError`'s posture for a broken `LockBackend` — a misconfigured or unreachable swapped PolicyBackend must refuse the write, not silently skip the consult. The POST-write recompute (after the write already landed, described above) is deliberately FAIL-OPEN — refusing after the fact cannot undo an applied write, so a recompute failure instead falls back to the pre-confirm preview value and records that fallback explicitly: the audit record's `policy_override_at_write` carries the stale preview value rather than an authoritative post-write one, and a sibling boolean, `policy_recheck_ok` (see `extra{}` below), is `false` precisely in this case — the write's own `--json` success payload also surfaces the fallback as a warning, so a copilot cannot mistake the stale value for an authoritative one. See `policy_recheck_ok` in "Audit primitive" below.
...
#### Caps-compose: an open implementation question (escalated, not decided here)

The original draft of this section described gate (c) as a hard "a model whose spend profile those caps forbid is a compose failure." That sentence described an ASPIRATIONAL check with no computable definition — `CostCaps` (`atomic_agents/policy/types.py`) has exactly two fields, `daily_usd` and `monthly_usd`, aggregate spend-over-TIME totals, not a per-model or per-call price ceiling; `_costs.PRICING` gives $/1M-token RATES, not an absolute cost; and a static config write (`--model <id>`) carries no token-volume signal to turn a rate into a dollar figure comparable against a daily/monthly cap. No pure function anywhere in the codebase maps `(model_id, CostCaps) -> forbid/allow`, unlike checks (a) and (b), which reuse real, tested functions. Per CLAUDE.md principle #13 (docs match reality, not aspiration) and the project's "verify before claim" discipline, this section now describes what PR1 actually does — CONSULT, never refuse on the value — rather than implying a hard gate exists. A precise `(model_id, CostCaps)` predicate, if wanted, needs its own explicit maintainer ruling before it is coded; it is not decided by this revision.
...
- (b) every prose paragraph, HTML comment, table, and the `provider:` line survive **byte-for-byte**;
...
This makes `set-model` the framework's **first surgical `model.md` field-writer.** Today `atomic_agents/_model.py` is READER-only. The only existing WRITER, `atomic_agents/profile/filesystem.py`'s `save_profile()` (`atomic_write(agent_root / "model.md", profile.model_md_raw)`), is a wholesale blob rewrite of the whole file's raw text — correct for its job (profile round-tripping) and NOT reusable here, since it has no concept of "change one heading's value, preserve everything else" and would require the caller to already have the full correct byte content in hand. `set-model` implements a NEW line-aware in-place value editor (`atomic_agents/manage/set_model.py`) following the established idiom (`_model.py`/`_roster.py`-style regex-targeted edits, no parse→re-serialize round trip), mirroring `govern`'s `_edit_governance_block` shape (ruled `surgical-writer-structure`) and regex-anchored on the same reader pattern `_model.py:79-80` uses, extended with a duplicate-heading guard (mirroring `govern`'s duplicate-key guard) so an ambiguous file is refused rather than silently edited on the wrong occurrence.

**Markup-style preservation (resolved — PRESERVE AS FOUND, matching M2's byte-faithful ethos).** The surgical writer preserves the operator's EXISTING value-wrapper markup (`` **`id`** ``/`**id**`/`` `id` ``/bare) exactly as found — it never normalizes to one canonical form. The writer's regex splits the wrapper markup into its own capture groups distinct from the value token, so a write replaces ONLY the id characters; the wrapper survives untouched either side of it.

**CRLF byte-fidelity (ruled `crlf-byte-fidelity-read`).** The reader mirrors `govern`'s `newline=''` read (disabling Python's universal-newline translation) so a CRLF-authored `model.md` round-trips byte-for-byte outside the touched value span — the same approach `govern._read_or_create_governance` already uses for `governance.md`.

### Audit primitive

A new `PRIMITIVE_MANAGE_SET_MODEL = "manage_set_model"` (defined alongside `PRIMITIVE_MANAGE_GOVERN` / `PRIMITIVE_MANAGE_RESTORE` in `atomic_agents/logs/types.py`), using the M8-pinned `RunRecord` shape as-is — this section does not re-pin the shape, only names the new primitive and what rides in `extra{}`. Each management verb mints its OWN dedicated primitive id (the RunRecord `extra{}` shape is what's shared across verbs, not the primitive id) — a fleet-wide "full management audit trail" query enumerates every manage-family primitive rather than filtering on a single constant:

- `model="n/a"`, `input_tokens=0`, `output_tokens=0`, `status="applied"` (refusals and dry-runs emit no record at all, per M8's pinned status vocabulary); `cost_usd` is OMITTED (not `null` — `RunRecord.to_dict()` drops `None` optionals) even though PRICING data was consulted for gate (a), so no phantom non-spend entry folds into `sum_cost_for_period()`'s fleet cost aggregations;
- `principal_id` — the resolved Principal, `LOCAL_PRINCIPAL` for the home user;
- `changed_fields` — `["default_model"]` for the PR1 `--model`-only write (widens to include `"fallback_model"` when #754 ships `--fallback`);
...
- `policy_override_at_write` — the BEST-EFFORT post-write-recompute Policy `get_effective_model()` value (see the "Composing with the runtime" note above — not a hard atomicity guarantee), or `null` when Policy has no opinion. Populated on every applied write so the audit record is self-sufficient for diagnosing "why didn't my model change take effect" without re-deriving possibly-since-changed Policy state (WARN-AND-WRITE ruling).
- `policy_recheck_ok` — `true` when `policy_override_at_write` came from the post-write recompute (the authoritative case); `false` when the recompute raised and the value fell back to the pre-confirm-wait preview consult instead. Lets a copilot auditing this record distinguish an authoritative post-write read from a stale-preview fallback without re-deriving Policy state itself. The `--json` success payload carries the same field (plus, when `false`, a fallback warning surfaced through `policy_override` even if there is no genuine Policy-vs-`model.md` conflict to report).

`created` (govern-specific — records that `governance.md` did not previously exist) does NOT apply here and is OMITTED from `extra{}` entirely (not emitted as a stray always-`false` key): per "Behavior" step 1, an absent `model.md` is a resolve-time refusal for `set-model`, not a create-and-fill path, so no `set-model` audit record is ever emitted with `created` present at all. Dual-scope append (per-agent `LogBackend` + fleet `_manage` scope, collapsing to one append under a shared distributed store) follows M8's backend-aware rule unchanged.
...
- **M9 composition refusals:** an unpriced `--model` (not in `_costs.PRICING`) refuses before write, non-zero exit, `model.md` unchanged; an unknown model id (`UnknownModelError`) refuses distinctly from an ambiguous one (`AmbiguousBackendError`) with a distinguishable structured `--json` reason for each — including a negative control proving the `ambiguous_backend` reason string never contains `--provider`; each of the two gates is strip-tested independently (per-invocation negative control, per the project's discipline) so a partial composition implementation cannot false-green.
- **WARN-AND-WRITE:** a Policy-overridden model (`get_effective_model` returns a conflicting non-`None` value) still APPLIES the write (exit `0`), surfaces the warning at both preview and post-write, and records `policy_override_at_write` in the audit — asserted as a positive control distinguishing WARN from every M9 refusal.
- **Surgical preservation invariant:** an applied `--model` write leaves the `cost_guardrails` yaml block byte-for-byte identical (hash-compared before/after); leaves every prose paragraph and HTML comment byte-for-byte identical; leaves the `provider:` line untouched.
- **Markup-style invariant:** a conformance test asserts markup is preserved AS FOUND across all four operator-markup variants the reader accepts (`` **`id`** ``, `**id**`, `` `id` ``, bare).
- **CRLF invariant:** a CRLF-authored `model.md` fixture round-trips byte-for-byte outside the touched value span.
- **Absence + duplicate + unparseable refusals:** `set-model`'s own `model_md_absent` check refuses when called directly against an agent_dir with no `model.md` (distinct from `govern`'s create-absent path) — this is a direct-API/TOCTOU safety net, UNREACHABLE end-to-end through the real CLI (the registry resolve refuses with `agent_not_found` first, per model.md-presence being the discovery predicate), so it is tested via a direct call to the internal write-path function, not through `run_set_model`'s full registry-resolve surface; a present file with no `## Default model` heading refuses distinctly, and this refusal IS reachable end-to-end; a file with more than one such heading refuses distinctly (the duplicate-heading guard); a heading with no parseable value token following it refuses distinctly.
- **Restore does not re-validate:** `--restore` against a snapshot whose model id is no longer in `_costs.PRICING` (deprecated since the snapshot was taken) still restores successfully, surfacing a non-blocking advisory rather than a refusal.

---

## Third verb — `manage apply-rec <rec-id>` (#727)

The #616 console-recommendation loop-closer. The Fleet Observability Console (spec/54) already computes `savings_cost` recommendations — "swap `<agent>` from `<current_model>` to `<candidate_model>`, save ~$X/mo" — but rendering a recommendation is not applying it; today an operator reads the card and hand-runs `set-model`. `apply-rec` closes that gap: it takes a recommendation identifier the console displayed, re-derives and re-validates that recommendation against CURRENT data, and — only if it still holds — drives the SAME `set-model` write routine (#726) an operator would have run by hand. `apply-rec`'s own "write" IS a `set-model` write through the S2 spine; it does not open a second write path onto `model.md`.

This makes `apply-rec` a **composition** verb, not a third independent editor: it composes the advisor's pure-compute recommendation recompute (`atomic_agents/advisor/recommend.py`) with `set-model`'s existing M9 composition chain and surgical writer (#726). Nothing in this section re-specifies `model.md`'s preservation contract, the manage lease (M11), the snapshot/restore pair (M3), or the exit-code ladder — `apply-rec` inherits all of it by delegating the actual write, exactly as `set-model`'s own spec text (§"Second verb") already anticipates: *"applying a console `savings_cost` recommendation is, mechanically, a `set-model` write with the recommended model id as `--model`."*

### Surface

```
atomic-agents manage apply-rec <rec-id> [options]
atomic-agents manage apply-rec <rec-id> --agent <name>
atomic-agents manage apply-rec <rec-id> --dry-run
```

- `<rec-id>` — a single opaque positional, the 12-hex-char identifier the console rendered on the `savings_cost` card (see "Rec-identity" below). The identifier is a recomputed hash, never a stored row, so there is no lookup, database, or file to go stale independent of the underlying data.
- `--agent <name>` — a scope-narrower, not a co-identifier: it filters the fleet-wide match universe down to one agent BEFORE matching `<rec-id>`. It does not change what a rec-id means and is never required — a bare `<rec-id>` searches the whole fleet.
- `--dry-run` — preview only (S2 step 2): shows the resolved `(agent, current_model → candidate_model)` swap AND the full `EvalHeadroom` (all four margins — `weighted_score_margin`, `pass_rate_margin`, `hard_fails`, `sample_n`), without writing. No write happens whether re-validation would pass or fail.
- `--yes` — apply without the interactive confirm (S2 step 3). Identical semantics to `govern`/`set-model`.
- `--json` — structured output (S3), including a structured reason on every refusal path introduced by this verb (`rec_no_longer_valid`, `rec_kind_not_applicable`, `rec_source_not_applicable`, `rec_guard_failed`), in addition to the full `set-model` M9 refusal vocabulary it composes with unchanged. Success payloads carry set-model's own shape plus `applied_from_rec` (`{rec_id, kind}`) and `safety` (the `EvalHeadroom` margins).
- `--agents-root` — fleet-scoped agent resolution, consistent with every other verb.

`apply-rec` has no `--show`/`--list-snapshots`/`--restore` of its own — it creates no new restorable state (M1 composition). Inspecting or rolling back the resulting `model.md` change uses `set-model --show` / `set-model --list-snapshots` / `set-model --restore` directly, against the SAME snapshot tree `apply-rec`'s delegated write populates.

### Behavior (the S2 routine, concretely)

1. **Resolve + validate.** Build the CURRENT match universe (`build_rec_match_universe(agents_root)` — see "The match universe" below; recommendations are NEVER persisted, so this call is unconditional and always fresh) and, when `--agent` is given, filter it to that agent first. Compute the rec-identity hash for every candidate in the (possibly filtered) universe and look for one matching `<rec-id>`.
   - No match → refuse `rec_no_longer_valid`.
   - A match whose `kind != "savings_cost"` → refuse `rec_kind_not_applicable`.
   - A match whose `.source` is outside the applicable-sources allowlist → refuse `rec_source_not_applicable` (see "Source restriction — the skeptic's guard").
   - A match whose `.safety.passed` is `False` → refuse `rec_guard_failed`.
   - Otherwise, delegate into `set-model`'s OWN step-1 resolve + M9 composition chain (PRICING/backend-resolution/policy-consult) against the matched `candidate_model` — a recommendation that named a model since deprecated from `_costs.PRICING`, or now claimed by more than one `LLMBackend`, is refused by `set-model`'s existing gates, unchanged, with `set-model`'s existing `error_type`s.

   All of this — the rec-match, the four apply-rec gates, AND the delegated `set-model` validation — happens BEFORE step 2, per M4. The order is load-bearing: a `<rec-id>` that hash-matches nothing gets `rec_no_longer_valid` before apply-rec ever looks at kind, source, or guard state; a `<rec-id>` that matches but is the wrong kind gets `rec_kind_not_applicable` before source or guard are even inspected. Each gate answers a genuinely different question, and the copilot-facing `error_type` must never blur two of them into one.
2. **Preview.** Render the SAME before→after `set-model` would render for `--model <candidate_model>` (the `## Default model` value line, everything else unchanged), plus the recommendation's own rationale and the full `EvalHeadroom` so the operator sees WHY the swap is recommended and how much quality headroom backs it, not only WHAT changes. Nothing is written. Human mode only — under `--json`, the same data travels in the `applied_from_rec`/`safety` payload keys instead of printed prose.
3. **Confirm.** Interactive y/n on a TTY, or `--yes`. Identical to `set-model` — no lease is held across this step (M11, inherited).
4. **Snapshot + atomic write.** Delegate into `set-model`'s own step 4 unchanged: acquire the per-agent manage lease, re-read `model.md` fresh, snapshot under `.config-snapshots/set-model/` (NOT a separate `apply-rec` snapshot namespace — see "The delegation seam"), write surgically, release the lease.
5. **Audit.** See "Audit" below — `PRIMITIVE_MANAGE_APPLY_REC` plus an `applied_from_rec` marker, carrying a `RunRecord` identifying that this write was rec-driven, not operator-typed.

### Rec-identity — deterministic recomputed hash (MAINTAINER-RULED, 2026-07-06; hash recipe finalized #727 Unit 2)

A `Recommendation` (`atomic_agents/advisor/recommend.py`) has no persisted identifier and no persisted row to look one up in — `recommend_fleet()` is a pure-compute recompute over 30-day JSONL windows, called fresh on every console render and every `apply-rec` invocation (spec/54; ZERO new LLM spend, ZERO state store). A `<rec-id>` is therefore, by ruling, a **deterministic hash of the recommendation's identifying fields** — `(agent, kind, candidate_model)`, the three `Recommendation` fields that together name "this specific swap for this specific agent" — NOT a stored value, NOT a database row, NOT a new sidecar file (composes M1: `apply-rec` MUST NOT introduce a management-state store to persist recommendation identities).

**Hash recipe (finalized, `canonical_rec_id()` in `atomic_agents/advisor/recommend.py`):** SHA-256 over the versioned, delimiter-safe canonical string `"v1\x1f{agent}\x1f{kind}\x1f{candidate_model_or_empty}"` (`\x1f` is the ASCII Unit Separator, chosen so a literal separator character can never appear inside a field and collide two distinct triples), truncated to the first 12 hex characters (the same short-prefix convention the console's `attention.py` alert-id already uses). `candidate_model` is encoded as an empty string for the non-model kinds (`governance`/`quality_report`), which is what makes those recs hash-reachable at all — see "The match universe" below. `source` is DELIBERATELY EXCLUDED from the hash: a swap's identity is the agent + kind + candidate model, not how the candidate was selected, so the SAME swap surfaced by two different selection paths still hashes to one id. `canonical_rec_id()` is the ONE shared helper — both the console's `_render_recommendations` and `apply-rec`'s match resolution import it directly — so the rendered rec-id and apply-rec's match universe can never diverge on what a given swap hashes to.

Concretely: the console renders a `<rec-id>` next to each `savings_cost` card by hashing the SAME three fields at render time; `apply-rec` independently rebuilds the match universe and hashes every current candidate's SAME three fields the SAME way; a match means "a recommendation for this exact agent/kind/candidate-model swap still holds today," and a non-match means the console's rendered rec has gone stale — the agent's usage repriced, the candidate model changed, or the underlying data moved enough that the current universe no longer produces that exact triple. **`rec_no_longer_valid` is a first-class, expected refusal, not an error condition** — a stale console card MUST NOT silently apply a swap the current data no longer recommends; this is the correct behavior for a system whose recommendations are recomputed, not committed.

### The match universe (RESOLVED — `build_rec_match_universe()`, #727 Unit 2)

`apply-rec` does NOT match against `recommend_fleet()` alone. `recommend_fleet()` is the CONSOLE feed — by spec/54 design it silently drops any `savings_cost` candidate whose no-quality-cost guard fails, and it never emits `quality_report`/`governance` alongside a hash a bare savings-only universe could resolve against. Matching only against it would make TWO of apply-rec's four gates structurally unreachable:

- a rec-id for a candidate that FAILED the guard would simply hash-miss into `rec_no_longer_valid` — `rec_guard_failed` could never fire, because the guard-failed candidate was never in the universe to match in the first place;
- a rec-id for a `governance`/`quality_report` recommendation is a real, console-rendered id (the console renders all three kinds, spec/54), but a `savings_cost`-only universe would hash-miss it too, so `rec_kind_not_applicable` could never fire — a copilot pasting a governance rec-id would get the wrong, uninformative refusal.

`build_rec_match_universe(agents_root)` (`atomic_agents/advisor/recommend.py`) closes both gaps: it is the UNION of `recommend_fleet()` (every kind, the console-visible set) and `derive_savings_candidates_fleet()` (`savings_cost` candidates INCLUDING guard-failed ones — the #727 Unit 1 candidate-vs-render split), deduplicated by `canonical_rec_id` (a passed savings candidate appears in both loaders; the `recommend_fleet()` copy is kept, since it is the exact object the console rendered). `compute_fleet_health` is computed ONCE and threaded into both loaders (Principle #6 — never score the fleet twice for one invocation). Both loaders resolve `RecommendationConfig` through the SAME `parse_recommendations(agents_root)` path the console itself uses, so apply-rec's verdicts never diverge from what the rendered card promised.

apply-rec reads the guard verdict as DATA off the matched candidate's `.safety.passed` field — it never imports `_eval_headroom` or any other advisor underscore-private directly (Principle #3 / TENSIONS T17 layering: a manage-layer verb composes with the advisor's public recompute surface, `build_rec_match_universe`/`recommend_fleet`/`derive_savings_candidates_fleet`, it does not reach past that surface into internals the advisor module owns). This is the fix to an earlier draft of this section, which said apply-rec "MUST re-run `_eval_headroom(...)`" directly — that would have been a layering violation AND would have made `rec_guard_failed` unreachable in practice, since `_eval_headroom` needs pre-loaded per-agent inputs (`eval_records`, `rubric_threshold`, `rec_config`) that only the shared fleet loader (`_load_fleet_agent_inputs`) assembles correctly; re-deriving them a second time in the CLI layer would risk exactly the window-alignment drift spec/54 MUST 11 exists to prevent. Freshness comes from `build_rec_match_universe` being an unconditional SAME-INVOCATION recompute, never a CLI-side re-derivation and never a render-time cache.

### Refusal vocabulary — four distinct refusals, four distinct retryability stories

- **`rec_no_longer_valid`** — no candidate in the (possibly `--agent`-scoped) match universe hashes to `<rec-id>`. EXPECTED, not an error — the console card is stale. Retryable: reload the console (or re-run apply-rec against the fresh universe) and pass the new rec-id.
- **`rec_kind_not_applicable`** — a match exists but its `kind` isn't `savings_cost` (see "Applicable rec kinds"). Not retryable via apply-rec — `quality_report`/`governance` recommendations have no mechanical apply, ever.
- **`rec_source_not_applicable`** — a match exists, is `savings_cost`, but its `.source` is outside PR1's allowlist (see "Source restriction — the skeptic's guard"). Not retryable via apply-rec today; the operator can still apply the swap by hand via `set-model --model` if they have independently validated the candidate's basis.
- **`rec_guard_failed`** — a match exists, is `savings_cost`, has an applicable source, but its no-quality-cost guard no longer passes. STOP and look at the agent's evals; do NOT blindly retry — retrying without investigating just re-hits this same refusal until the underlying quality signal genuinely recovers.

These four are deliberately never merged: the third (`rec_source_not_applicable`) composes on top of the two the maintainer had already ruled (a match-shape refusal and a guard-shape refusal) rather than being folded into either, because it answers "is this candidate's SELECTION BASIS one apply-rec can vouch for" — a question distinct from both "does a candidate exist" and "does the existing candidate still pass its quality guard." Folding it into either would collapse two operator actions ("reload the console" vs. "this will never auto-apply, use `set-model` by hand if you trust it") into one refusal a copilot cannot react to correctly.

Past all four gates, apply-rec delegates into `set-model`'s OWN M9 composition chain (PRICING/backend-resolution/policy-consult) against the matched `candidate_model`, unchanged, with `set-model`'s existing `error_type`s (`unpriced_model`/`unknown_model`/`ambiguous_backend`) — a recommendation that named a model since deprecated from `_costs.PRICING`, or now claimed by more than one `LLMBackend`, refuses through those gates exactly as a hand-typed `set-model --model` invocation would. apply-rec does not re-run or shortcut them.

### Source restriction — the skeptic's guard (RESOLVED, #727 Unit 2)

A `savings_cost` `Recommendation.source` is either `"default_same_family"` (the baked-in conservative downgrade map — spec/54 §4) or `"operator_configured"` (an operator's own `work_type_allowed_models` override in `targets.md`). PR1 applies **`default_same_family`-sourced candidates only** — `_APPLICABLE_REC_SOURCES = frozenset({"default_same_family"})` in `apply_rec.py`.

`operator_configured` is excluded BY DESIGN, not as an oversight: apply-rec has no way to re-validate WHY an operator listed a given candidate under `work_type_allowed_models` — the operator may have hand-picked it for reasons (a prior manual eval, a vendor recommendation, a cost-ceiling constraint) that `_eval_headroom`'s generic no-quality-cost guard cannot see or confirm. Silently auto-applying an operator-configured candidate would extend apply-rec's "only apply what we can vouch for" promise past what it can actually vouch for. This is a documented, intentional, conservative gap for PR1 — an operator can still apply an `operator_configured` candidate by hand via `set-model --model`, exactly as they could before `apply-rec` existed; PR1 just doesn't automate that path yet.

A future `measured_scorecard` source (does not exist in code today — see "Applicable rec kinds"'s forward note, #649/#644-child-D) is the PRIMARY target this allowlist is designed to widen for later, once a re-validation path for a scorecard-backed candidate (re-checking the scorecard result rather than, or in addition to, `_eval_headroom`) is itself designed. Widening `_APPLICABLE_REC_SOURCES` is a small, isolated change when that day comes; PR1 deliberately does not speculatively build the wider gate now.

### The delegation seam (RESOLVED — `apply_set_model_write()`, #727 Unit 2)

`apply-rec` does NOT duplicate `set-model`'s surgical `model.md` writer, its lease/snapshot/restore machinery, or its M9 composition chain — reimplementing any of those would violate the "compose, never bypass" discipline (M9) the same way a hand-rolled model-write bypassing `set-model` entirely would. The mechanism, decided in favor of the lower-level shared entry point over an `args`-namespace synthesis: `set_model.py`'s former private `_run_set_model` write function is promoted to a PUBLIC `apply_set_model_write()`, with `_run_set_model = apply_set_model_write` kept as a module-level backward-compatible alias. `set-model`'s own CLI dispatch (`run_set_model`) calls it exactly as before; `apply-rec` calls the SAME function directly, passing `matched.agent`/`matched.candidate_model` in place of `args.agent`/`args.model` — never touching `argparse.Namespace` at all.

Four OPTIONAL keyword-only parameters, all defaulting to reproduce `set-model`'s own CLI-path behavior byte-for-byte, are the entire seam:

- `audit_primitive` (default `PRIMITIVE_MANAGE_SET_MODEL`) — lets `apply-rec` mint its OWN primitive (`PRIMITIVE_MANAGE_APPLY_REC`) on the `RunRecord` it delegates into.
- `audit_extra` (default `None`) — merged into `RunRecord.extra{}` LAST, so a delegating caller's marker can never be silently clobbered by a set-model-native key (or vice versa — a genuine collision is a caller bug, not a runtime hazard this seam papers over).
- `json_extra` (default `None`) — merged into the `--json` success payload the same way, letting `apply-rec` attach `applied_from_rec`/`safety` without touching `_emit_json_success`'s set-model-native keys.
- `human_preamble` (default `None`) — a zero-arg callable invoked right before the human-mode preview prints (never under `--json`, where the same data travels in `json_extra` instead), letting `apply-rec` print its rec/EvalHeadroom context BEFORE set-model's own default-model diff.

The snapshot lands in the SAME `.config-snapshots/set-model/` tree (no separate `apply-rec` snapshot namespace), and `set-model --restore`/`--show`/`--list-snapshots` work identically on a rec-applied write as on a hand-typed one — because it IS a `set-model` write, produced by the identical write function.

### Audit (RESOLVED — new dedicated primitive + marker, #727 Unit 2)

`PRIMITIVE_MANAGE_APPLY_REC = "manage_apply_rec"` (`atomic_agents/logs/types.py`, alongside `PRIMITIVE_MANAGE_GOVERN`/`PRIMITIVE_MANAGE_SET_MODEL`/`PRIMITIVE_MANAGE_RESTORE`) — a NEW dedicated primitive, not the existing `PRIMITIVE_MANAGE_SET_MODEL` primitive reused with a marker alone. The `RunRecord.extra{}` additionally carries an `applied_from_rec` marker (`{"rec_id": <the matched rec-id>, "kind": "savings_cost"}`) so a copilot reading a single record does not have to cross-reference the primitive id to learn it was rec-driven. The dedicated-primitive choice makes "how often do rec-driven applies happen vs. hand-typed `--model` swaps" and "what is the adoption/refusal rate of console recommendations" directly queryable by `primitive=manage_apply_rec` alone, without also filtering on the marker — the marker is redundant-but-convenient at the single-record level, the primitive is what makes the fleet-wide signal a one-field `LogQuery`.

The `RunRecord` SHAPE is M8-pinned IDENTICAL to `set-model`'s own applied-write shape (`status="applied"`, `model="n/a"`, `input_tokens=0`/`output_tokens=0`, `principal_id`/`changed_fields`/`before`/`after`/`snapshot_path` in `extra{}`) — this section does not re-pin it, matching the precedent `set-model`'s own "Audit primitive" subsection sets ("the primitive id is explicitly NOT one of the pinned-identical parts"). A fleet-wide "full management audit trail" query still matches the whole `manage_*` primitive family; a "rec-driven applies only" query matches `manage_apply_rec` specifically.

### Applicable rec kinds

PR1 of `apply-rec` applies **`savings_cost` recommendations only** — the one kind with a mechanical, unambiguous "apply" action (a model swap). `quality_report` (surface an eval/tuning-report signal) and `governance` (governance.md is absent/invalid) recommendations are, and remain, advisory-only — there is no mechanical "apply" for "go read this tuning report" or "go write governance.md by hand" (the latter has its own editor, `manage govern`, which an operator drives directly; `apply-rec` does not chain into `govern`). Calling `apply-rec` with a `<rec-id>` that resolves to a `quality_report` or `governance` recommendation MUST refuse cleanly (`rec_kind_not_applicable`) rather than silently no-op or attempt an unsupported write. This gate is REACHABLE (not merely defined) precisely because "The match universe" above unions in every kind — a savings-only universe could never surface a `governance`/`quality_report` rec-id to test this gate against.

**Forward note (#649 / #644-child-D coordination).** A `savings_cost` recommendation's underlying pricing signal is, today, a straight reprice of 30-day actual usage at a cheaper same-family sibling's PRICING rate (spec/54), surfaced through exactly one applicable `source` in PR1 (see "Source restriction" above). The Model Testing & Capability Scorecard design (#644-child-D, spec/55 cross-reference pending) anticipates a SECOND `savings_cost` source — a `measured_scorecard`-backed recommendation, where the candidate model has been validated against a ground-truth test set rather than assumed same-family-safe. `apply-rec`'s rec-identity hash, its match universe, and its source allowlist are designed against TODAY's single applicable source; whether a future `measured_scorecard`-sourced rec needs a distinct re-validation path (e.g. re-checking the scorecard result instead of, or in addition to, `.safety.passed`) is explicitly NOT decided here — flagged so a future scorecard arc revisits this section rather than silently drifting from it.

### Accepted consequences (documented, not fixed — mirrors M11's accepted-window style)

- **TOCTOU across the confirm prompt.** The four apply-rec gates (match, kind, source, guard) and `set-model`'s M9 chain are all checked BEFORE the confirm prompt (S2 step 3, per M4) — an unbounded interactive wait can follow. apply-rec does NOT re-validate any of the five gates under the manage lease at write time; the delegated `apply_set_model_write` re-reads `model.md` fresh inside the lease (the #709 lost-update fix, inherited unchanged), but it re-reads the FILE, not the recommendation. A recommendation that stopped holding during an unbounded confirm wait still applies. This mirrors `set-model`'s own accepted staleness window for its Policy-override preview (see "Composing with the runtime") and is accepted for the identical reason: re-validating under the lease would mean running the fleet-wide recompute INSIDE a lock every OTHER manage write on the agent is blocked behind, trading a narrow staleness window for a much worse availability hazard.
- **Full-fleet recompute to apply one rec.** `build_rec_match_universe` runs `compute_fleet_health` plus both recommendation loaders over the WHOLE fleet to resolve a single `<rec-id>`, even with `--agent` scoping (the scope narrows the search, not the computation — every agent's health is still scored before filtering). This is pure, zero-LLM-spend compute (spec/53/spec/54), so the cost is CPU/IO time, not dollars; resist the temptation to build a per-agent recompute shortcut — it would require duplicating `_load_fleet_agent_inputs`'s window-alignment discipline (spec/54 MUST 11) for one call site, exactly the kind of sibling-loader drift risk `_load_fleet_agent_inputs` was built to eliminate.
- **`RecommendationConfig` coherence via the shared path.** Both `recommend_fleet()` and `derive_savings_candidates_fleet()`, inside `build_rec_match_universe`, resolve `rec_config` through the SAME `parse_recommendations(agents_root)` call the console itself uses — so an operator-edited `targets.md` (floors, `work_type_allowed_models`) is honored identically by both surfaces. There is no separate apply-rec-specific config path to drift out of sync.

### Conformance test outline (apply-rec additions)

Mirrors the shape of the top-level "Conformance test outline" section and `set-model`'s own additions to it; these are the `apply-rec`-specific additions, not a replacement (implemented in `tests/test_manage_apply_rec.py` + `tests/test_manage_spine.py`):

- **Rec-match:** a `<rec-id>` recomputed from a currently-valid, `default_same_family`-sourced, guard-passing `savings_cost` recommendation resolves and applies (exit `0`, `model.md` written); a `<rec-id>` that matches no current recommendation refuses `rec_no_longer_valid`, non-zero exit, no write (positive + negative control pair).
- **Guard re-validation:** a recommendation whose `.safety.passed` was `True` when first computed but is made `False` by injecting a fresh hard-fail before `apply-rec` runs refuses `rec_guard_failed`, distinct from `rec_no_longer_valid` and distinct from any `set-model` M9 refusal — asserted as three separately strip-tested negative controls (per the project's per-invocation negative-control discipline), so a partial re-validation implementation cannot false-green.
- **Kind gate:** a `<rec-id>` resolving to a `quality_report` or `governance` recommendation refuses `rec_kind_not_applicable` BEFORE any `set-model` delegation is attempted; a `<rec-id>` resolving to `savings_cost` proceeds past this gate (positive control).
- **Source gate:** a `savings_cost` candidate whose `source` is `operator_configured` (constructed via a `work_type_allowed_models` config, or a monkeypatched candidate) refuses `rec_source_not_applicable`, exit `1`, no write.
- **Delegation fidelity:** an applied `apply-rec` write is byte-identical in its `model.md` result and snapshot location to a hand-typed `set-model --model <candidate_model>` run against the same fixture and starting state — asserted by running both paths and diffing the outcomes.
- **Audit primitive:** the applied write's `RunRecord` has `primitive == PRIMITIVE_MANAGE_APPLY_REC` and an `extra["applied_from_rec"]` marker carrying the rec-id and kind; the `RunRecord` SHAPE otherwise matches `set-model`'s own applied-write shape exactly.
- **M9 composition still applies:** a recommendation naming a `candidate_model` that has since been removed from `_costs.PRICING` (deprecated after the recommendation's data was generated but before `apply-rec` runs) refuses via `set-model`'s existing `unpriced_model` gate, unchanged — `apply-rec` does not shortcut or duplicate this check.
- **`--dry-run`:** shows the resolved swap and the full `EvalHeadroom` (`safety` key present under `--json`) without writing.
- **`--json` refusal shape:** each of the four apply-rec-introduced refusals emits `{ok: false, error_type: <the type>, ...}` with the stable key.
- **Spine inheritance:** `--yes`/exit-code ladder/manage lease/agent-busy behave identically to a direct `set-model` invocation, verified against a focused subset of the SAME spine test suite (the lost-update / lease-scope guarantees are inherited via the delegated write, so a full re-test of every spine scenario is unnecessary — set-model's own spine suite already covers them at the write-function level `apply-rec` calls into).
...
- **Epic #606** — Agent Fleet Platform (observe → manage → interact); #624 (this foundation), #609 (the `govern` verb), #726 (the `set-model` verb), #727 (the `apply-rec` verb).
...
- **#754/#755** — deferred `set-model` follow-ups: `--fallback` (same M9 chain as `--model`, no reduced bar) and `--provider` (ambiguity disambiguation + the insert-when-absent `provider:`-line writer).
- **#727 Unit 1** — the advisor candidate-vs-render split `apply-rec` reads from: `derive_savings_candidates()`/`derive_savings_candidates_fleet()` (guard-failed candidates surfaced as data) and `canonical_rec_id()` (the shared rec-id hash both the console and `apply-rec` import), in `atomic_agents/advisor/recommend.py`.
- **#649 / #644-child-D** — Model Testing & Capability Scorecard; the anticipated `measured_scorecard` savings-rec source `apply-rec`'s source allowlist is designed to widen for later (see "Source restriction — the skeptic's guard").
- **TENSIONS T17** — core↔extension boundary; `manage/` is an extension package and routes PRICING lookups through `atomic_agents.core_api.get_model_rates()`, never `_costs.PRICING` directly; `apply-rec` composes with the advisor's PUBLIC recompute surface only, never its underscore-privates (the same layering discipline applied one level deeper).
...
- **spec/54** Fleet Console Recommendations Engine — the `Recommendation`/`EvalHeadroom` dataclasses, `recommend_fleet()`/`derive_savings_candidates_fleet()`, and the composite conjunctive no-quality-cost guard `apply-rec` reads the verdict of as data.
