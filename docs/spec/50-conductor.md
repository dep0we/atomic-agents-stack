...
- **Single-writer per `conductor_run_id` (PR1).** `run()` takes **no run-level lock**. Two concurrent `run()` calls on the same `conductor_run_id` would double-dispatch stages and defeat the tree-cap (each re-sums spend independently; `IN_FLIGHT` idempotency is not a hard stop). PR1 scope **excludes** concurrency — the caller MUST serialize invocations per `conductor_run_id`. The run-level lock + conflict serialization land in **PR3 (#582)**; this is documented, not built here.
