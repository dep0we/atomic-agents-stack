...
> **Status:** LOCKED at PR 5 (v1.0.0), amended post-v1.0 for GHSA-xhcr-cqfr-m3hv (transport security + spawn gate). HTTP write paths shipped. The parametrized conformance suite (`tests/test_mcp_server_registry_conformance.py`) asserts MUSTs 1-10 across both reference implementations; MUST 11 (HTTP transport scheme gate) is asserted in `tests/test_mcp_server_registry_http_backend.py` and MUST 12 (`MCPClientPool` spawn gate — a pool-layer invariant, not a backend method) in `tests/test_mcp.py`.
...
- **Post-v1.0 security amendment (GHSA-xhcr-cqfr-m3hv).** Added MUST 11 (transport security / scheme gate) and MUST 12 (spawn gate / command allowlist). 10 → 12 MUSTs. See `atomic_agents/mcp_registry/http.py` and `atomic_agents/mcp.py` for implementation.
...
2. **Environment variables:**
   ...
   - `ATOMIC_AGENTS_MCP_SERVER_REGISTRY_ALLOW_HTTP`: set to `"1"` to allow cleartext HTTP connections to non-loopback catalog hosts (MUST 11 opt-in). Only `"1"` is accepted; absent, empty, `"true"`, `"yes"` all mean deny. Loopback hosts (`127.0.0.0/8`, `::1`, `localhost`) are always allowed; this flag is only relevant for non-loopback hosts.
...
## Implementer contract (10 backend MUSTs + 2 framework-security MUSTs)

Backends implementing `MCPServerRegistryBackend` MUST satisfy **MUSTs 1-10**. The conformance suite at `tests/test_mcp_server_registry_conformance.py` parametrizes across registered backends and asserts each of these ten.

**MUST 11 and MUST 12 are framework-security invariants, not per-backend method contracts** — they are listed here because they close the same advisory (GHSA-xhcr-cqfr-m3hv) and gate the same RCE path, but they bind specific layers rather than every backend:

- **MUST 11** binds `HTTPMCPServerRegistryBackend` only (transport security; the filesystem backend has no network transport, so it is trivially N/A). Asserted in `tests/test_mcp_server_registry_http_backend.py`.
- **MUST 12** binds `MCPClientPool` (the subprocess spawn layer), NOT any `MCPServerRegistryBackend` implementation — a `FilesystemMCPServerRegistryBackend` has no spawn site, so "all backends MUST satisfy MUST 12" would be unsatisfiable for it. It is a pool-layer invariant and cannot be asserted by the backend-parametrized conformance suite. Asserted in `tests/test_mcp.py`. (Principle #3 — the registry-backend layer and the pool spawn layer are different layers; the gate lives where the subprocess is spawned.)

The backend MUST count is 10 because the static-vs-dynamic capability distinction (Decision 6) is novel relative to the 11 prior arcs (which all had static-equals-dynamic capabilities) and requires two additional MUSTs (MUST 3 extended to cover the static/dynamic distinction; MUST 10 added for `load_all_mcp_servers()` consistency) beyond the 8-MUST range of prior arcs. MUST 11 and MUST 12 were added post-v1.0 to close GHSA-xhcr-cqfr-m3hv (cleartext HTTP catalog MITM → local RCE).
...
**MUST 11: Transport security (scheme gate).** `HTTPMCPServerRegistryBackend` MUST refuse cleartext HTTP catalog connections unless the catalog host is loopback (`127.0.0.0/8`, `::1`, literal `localhost`). Non-loopback HTTP requires explicit operator opt-in via `ATOMIC_AGENTS_MCP_SERVER_REGISTRY_ALLOW_HTTP=1` (only the string `"1"` counts; absent or any other value = deny). The constructor kwarg `allow_http_non_loopback: bool | None` wins over the env var when provided. If the scheme gate fails, every public method that issues network I/O — the read paths `list_mcp_servers()`, `load_mcp_server()`, `load_all_mcp_servers()`, `validate()` AND the write paths `install()`, `uninstall()` — MUST raise `ValueError` with an operator-readable message naming the opt-in env var. Each MUST call the scheme gate BEFORE its `_ensure_probed()` so a refused scheme blocks even when the capability cache is already warm (a malformed-too-far-to-classify URL MUST also fail closed, not be granted the non-loopback exemption). `__init__` MUST NOT raise (MUST 2 side-effect-free construction); the gate fires at the first I/O call. The loopback check MUST use `urlparse().hostname` (NOT `.netloc`) to resist userinfo-injection attacks (`http://127.0.0.1@evil.com/` has hostname `evil.com`, not `127.0.0.1`). The `_allow_http_non_loopback` flag MUST be resolved once at construction time (NOT per-call) to avoid TOCTOU races on env var mutation between calls. **Rationale:** cleartext HTTP catalog connections allow a network MITM to inject arbitrary `command` fields into `MCPServerSpec` responses, which `MCPClientPool` executes as subprocesses. Loopback traffic cannot be intercepted by a network adversary; it is treated as trusted by default. All non-loopback HTTP requires explicit operator acknowledgement. Conforms to GHSA-xhcr-cqfr-m3hv. See `atomic_agents/mcp_registry/http.py:_assert_scheme_allowed`.

**MUST 12: Spawn gate (command-basename allowlist).** `MCPClientPool` MUST validate the `command` basename of every `MCPServerSpec` against an operator-configurable allowlist before spawning any subprocess. The default allowlist is `{"npx", "uvx", "python", "python3", "node", "docker"}`. Operators may replace (not extend) the allowlist by adding a `## Allowed commands` section to the agent's `mcp.md` file; the section body is a newline-separated list of bare command names. An empty `## Allowed commands` section is explicit deny-all (not "use default"). If the command basename is not in the allowlist, `MCPClientPool.connect_all()` MUST raise `MCPCommandNotAllowed` (subclass of `MCPServerConnectFailed`) before `StdioServerParameters` is ever constructed — no subprocess is spawned. The `MCPCommandNotAllowed` exception MUST propagate out of `connect_all()`; it MUST NOT be caught by the surrounding `MCPServerConnectFailed` handler (fail-closed). The allowlist is resolved once at `MCPClientPool.__init__` time (operator-passed kwarg `allowed_commands: frozenset[str] | None`; `None` means use default). **Rationale:** even with the scheme gate (MUST 11), a compromised catalog could inject a command field into a spec that passes the scheme check. The spawn gate is defense-in-depth — it limits the attack surface to commands the operator explicitly trusts, regardless of how the spec was obtained. An injected `"command": "bash"` or `"command": "curl"` is blocked at spawn time. **Honest scope limit:** the gate blocks DIRECT shell/exec-binary injection, but it does NOT prevent code execution through an allowlisted *interpreter* invoked with a code-exec flag — `python -c '...'`, `node -e '...'`, etc. are equivalent to a shell for an attacker, and `python`/`python3`/`node` are in the default set. `args` are validated only for path-traversal, not for code-exec flags. Operators handling untrusted catalogs SHOULD narrow `## Allowed commands` to the specific runner(s) they need and/or pin those servers to bare-name commands their catalog emits. Args-level hardening (rejecting `-c`/`-e`-style flags) is tracked as a follow-up ([#408](https://github.com/dep0we/atomic-agents-stack/issues/408)). See `atomic_agents/mcp.py:MCPCommandNotAllowed`, `_check_command_allowlist`, `parse_mcp_allowed_commands`, `parse_mcp_md_with_policy`. Conforms to GHSA-xhcr-cqfr-m3hv defense-in-depth.
...
**Spec:** `docs/spec/36-mcp-server-registry-backend.md` LOCKED at 10 MUSTs (amended to 12 MUSTs post-v1.0 for GHSA-xhcr-cqfr-m3hv).
