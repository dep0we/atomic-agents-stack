...
There are no `ranking_*_weight` fields. `recommend_fleet()` ranks recs solely by `abs(projected_points_delta)` (§7 Option 1); a weighted ranking scheme would be config nothing reads. "Pick the option that adds fewer concepts."
