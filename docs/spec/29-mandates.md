...
**Atomic emission of `mandate_used` (plan-subagent PR 3b Risk E):** when a mandate-citing action emits a cost event, the framework MUST emit the sibling `mandate_used` lifecycle event in the same atomic-append batch (or transactionally on SQL backends) — NOT from `agent.py` after the cost write returns. Cross-module coupling (cost module knowing about mandates) is the right trade vs the race the other ordering creates. The cost-write path checks `if record.mandate_id is not None: emit_mandate_used(...)` as part of the same `LogBackend.append()` call.
