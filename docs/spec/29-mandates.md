...
All three reads share **one cap-gated rule** (#512). Each helper takes a `cap_active` parameter that the caller sets from the relevant **effective** cap presence (`_step7` passes `cap_active=token_cap_active`; `_step8` passes `cap_active=external_cap_active` — each is the mandate-declared cap **or** a Policy-composed effective cap; the cap-exceeded verdict re-reads pass the same per-dimension flags). When `cap_active=True` and a genuine read-failure occurs, the helper raises `_MandateCostUnreadable` and `evaluate()` returns BLOCK with reason `mandate_cost_unreadable`. When `cap_active=False`, the blind read degrades instead: the two SUM helpers return `$0.00`, and `_project_token_cost` falls back to its non-zero conservative default (`expected_cost_per_call_usd` or `$0.10`, which *over*-projects). Failing closed where no cap exists would spuriously block unconstrained agents with nothing to protect.
...
The *never-under-report-spend* invariant is preserved by this cap-gating: under-reporting prior spend only matters against an **active** cap that the summed value gates. A mandate with only an escalation threshold and **no** cap is the motivating case — its escalate decision uses the projection, and the cumulative is compared only against absent caps, so a blind SUM read there gates nothing. **#512 resolved** the residual over-block that the pre-#512 unconditional-fail-closed SUM rule produced in that configuration by aligning the two SUM helpers with `_project_token_cost`'s cap-gating: the escalation-only-no-cap configuration now degrades to `$0.00` (ALLOW) rather than over-blocking, while every capped configuration still fails closed (never leaks against a real budget).
