...
> **Versioned normative addendum (v2, issues #506/#512).** This subsection is a normative gate-site posture rule on the cost-read surface. It is **outside** the numbered Implementer-Contract MUST tally below (it constrains the live mandate spend-gate's read behavior, not a `MandateBackend` storage/recovery capability) — mirroring the spec/22 read-failure addendum (#497) and the spec/41 addendum (#483).

When any of the prior-spend / baseline-projection read helpers (`_sum_prior_token_cost`, `_sum_prior_external_cost`, and `_project_token_cost`) raises a genuine read-failure (`LogBackendReadError`, `OSError`, `sqlite3.DatabaseError`) **and the read gates an active control**, `evaluate()` MUST return BLOCK with reason `mandate_cost_unreadable` rather than treating the unread spend as `$0.00` (fail-open). The active control differs per read: a token cap for `_sum_prior_token_cost`; an external cap for `_sum_prior_external_cost`; a token cap **or** an escalation threshold for the baseline projection `_project_token_cost` (which feeds both the cap check and the step-9 escalation gate). The reads are **control-gated** (#512): where the read gates no active control, the blind read degrades (SUM helpers → `$0.00`; projection → conservative default) rather than BLOCK — failing closed where there is nothing to protect would spuriously block unconstrained agents (see the cost-read fail-closed gating rule, spec/09 §"Cost-read error posture", and `feedback_fail_closed_only_where_theres_something_to_protect`). This is the **read-side twin** of the §"Crash recovery for reservations" write-side "over-report, never under-report" rule — two faces of one *never-under-report-spend* invariant: when the gate cannot verify prior spend, it MUST refuse, never assume zero. This posture covers the full read surface — the budget sums consumed in steps 7-8 **and** the cap-exceeded verdict sums (`_sum_prior_token_cost` / `_sum_prior_external_cost`) re-read when composing the BLOCK verdict — not steps 7-8 alone.
...
Each read is **gated on the control it feeds** (#512). The helpers take a `cap_active` parameter. `_step7` passes `cap_active=token_cap_active` to the prior-spend SUM (which feeds only the cumulative-vs-cap check) and a separate `projection_cap_active=token_cap_active or has_token_escalation_threshold` to the baseline projection `_project_token_cost` (which feeds the cap check **and** the step-9 escalation gate); `_step8` passes `cap_active=external_cap_active`; the cap-exceeded verdict re-reads pass the same per-dimension SUM flags. Each cap flag is the mandate-declared cap **or** a Policy-composed effective cap. When the relevant flag is `True` and a genuine read-failure occurs, the helper raises `_MandateCostUnreadable` and `evaluate()` returns BLOCK with reason `mandate_cost_unreadable`. When it is `False`, the blind read degrades instead: the two SUM helpers return `$0.00`, and `_project_token_cost` falls back to its non-zero conservative default (`expected_cost_per_call_usd` or `$0.10`, which *over*-projects). Failing closed where the read gates no active control would spuriously block unconstrained agents with nothing to protect.

"Cap" in the `cap_active` flags means the **effective** cap = `MIN(mandate-declared constraint, Policy-composed cap)`. A Policy-composed effective cap (`_policy_effective_caps` — a dimension-agnostic daily/monthly USD cap that `_min_or_other` composes onto **both** the token and external budgets) **does** raise `cap_active`. Gating on the mandate-declared cap alone would let a blind SUM read degrade to `$0.00` and fail **open** against a live Policy budget, breaking the *never-under-report-spend* invariant against a real cap. The cap-exceeded re-read passes the **per-dimension** effective flag (`token_cap_active` / `external_cap_active`), not a literal `True`: a non-empty `caps_exceeded` proves *at least one* cap was exceeded, not that both dimensions are capped — so the token re-read fails closed only when `token_cap_active`, the external only when `external_cap_active`. Hardcoding `True` would over-block the *uncapped* dimension, flipping its truthful `mandate_cap_would_exceed` verdict to `mandate_cost_unreadable` and falsifying the audit reason for a dimension with no cap to protect (`feedback_fail_closed_only_where_theres_something_to_protect`). Only the genuinely-uncapped case (no mandate cap **and** no Policy cap) degrades to `$0.00`.

The mandate-declared cap names are: **token caps** `daily_token_usd` / `monthly_token_usd` / `cumulative_token_usd` (raise `has_token_cap`) and **external caps** `daily_external_usd` / `monthly_external_usd` / `cumulative_external_usd` / `per_action_max_usd` (raise `has_external_cap`). Note `per_action_max_usd` is an *external* per-action cap evaluated against the projection, not the prior-spend SUM — it is a forward-compat name guarded by `hasattr` and not yet a field on the stock `MandateConstraints` dataclass.

The *never-under-report-spend* invariant is preserved by this control-gating: under-reporting a read only matters against an **active** control that the read gates. A mandate with only an escalation threshold and **no** cap is the motivating case, and the two reads there diverge by design:

- Its **prior-spend SUM** feeds only the cumulative-vs-cap check, and the cumulative is compared only against absent caps — so a blind SUM read gates nothing and degrades to `$0.00`. **#512 resolved** the residual over-block the pre-#512 unconditional-fail-closed SUM rule produced in exactly this configuration.
- Its **baseline projection** feeds the step-9 escalation gate, which **is** an active control. So a blind projection read fails closed (BLOCK `mandate_cost_unreadable`) — otherwise it would degrade to the conservative default and could silently **suppress** an escalation a truthful (high-baseline) read would have fired, defeating an operator's "require approval above `$X`" control (#512).

So the escalation-only-no-cap configuration is fail-SAFE on both reads: the SUM degrades (no cap to protect) while the projection fails closed (the escalation gate is protected). Every capped configuration still fails closed on both reads (never leaks against a real budget).
