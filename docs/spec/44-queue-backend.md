...
> **Status:** LOCKED (issue #428, LOCK PR; MUST 13 `enqueue()` RE-LOCK: #582 PR3 (2026-06-27); MUST 13 validator hardened to reject leading-dot names: #582 PR3 R3 (2026-06-28)). 143 tests collected across `test_queue_backend_conformance.py` (68, +6 for enqueue round-trip, atomicity, path-separator strip-RED, null-byte rejection, the claim_next orphan-producer-temp skip, and leading-dot item_name rejection) and `test_queue_filesystem.py` (75). All 13 Implementer Contract MUSTs are individually test-covered. Registered in the shared #379 export conformance harness (`test_export_protocol_conformance.py`) and the capability-advertisement harness (`test_export_capability_advertisement.py`). The SCAFFOLDING-ONLY designation is RETIRED as of #582 PR3: `enqueue()` is the first producer primitive and is wired by the conductor's (write-only / advisory) conflict-queue mechanism. Runtime adoption of `claim_next` for the cascade use-case remains deferred to #469. Closes TENSIONS T4.
...
**The filesystem backend's security boundary is `project_root`, not the internal queue subdirectories.** This is the load-bearing statement that bounds what `FilesystemQueueBackend` does and does not defend, and it determines which deployments it is appropriate for.

**What is defended (the perimeter).** No queue operation may read, write, move, or export a file whose real location is OUTSIDE `project_root/queue/`. This is the genuinely dangerous class — host-byte exfiltration, writing outside the project, the `#426`/`#427` ancestor-escalation pattern. It is enforced by a single sound invariant rather than a stack of point-checks: every operation that touches a known work item (`claim_next`, `release`, `move_to_dead_letter`, `recover`'s reclaim) MUST require the work file's RESOLVED path to equal EXACTLY its expected canonical location under the resolved queue root (`claimed/<lease_token>/<original_name>` or `queued/_recovered/<lease_token>/<original_name>` for finalization; `queued/<role>/<name>` for a claim candidate) AND that the leaf is a regular file, not a symlink. Exact-canonical-equality (not "is under") is what makes the invariant sound: it subsumes containment, source-state, basename match, symlink-leaf rejection, AND symlinked-PARENT rejection (a symlinked ancestor makes `resolve()` diverge from the expected canonical path) in one check. `FilesystemQueueBackend._require_canonical_source()` is that chokepoint; it raises `PathTraversalError`, which every caller catches and fails SOFT. `export()` enforces the same perimeter per-leaf (resolve + `is_relative_to(queue_root)` + skip symlink leaves) because it enumerates arbitrary durable files rather than a known canonical path.

**What is NOT defended (within-queue state integrity).** An actor with write access to `project_root/queue/` is INSIDE the trust boundary. Such an actor can corrupt queue state directly — write arbitrary work files, delete claims, rewrite sidecars — so defending the internal state machine (e.g. "dead-lettered work can never re-enter `done/`") against that actor is defending a boundary that does not exist: a `queue/`-writer who plants a symlink to shuffle state could equally just write the target file. The backend therefore does NOT treat within-queue state manipulation by a `queue/`-writer as a vulnerability. (The one within-queue rule that IS enforced is the spec/40 export boundary — `claimed/` and `.lease.json` are excluded from the portable snapshot — because that is a portability CONTRACT, not an attacker boundary; symlink leaves are skipped in `export()` so the contract holds structurally.)

**The right tool for an adversarial deployment.** A deployment with co-located untrusted actors and real privilege boundaries (the multi-host / fleet case named in TENSIONS T4) is NOT served by hardening the filesystem backend against an attacker who owns the directory — that is not achievable for a plain-files store. It is served by registering a backend with a REAL authorization boundary: a Postgres / Redis / SQS `QueueBackend` whose access control is database grants or IAM, not filesystem symlinks. This is the swappable-protocol thesis doing its job: the filesystem backend is correct and sound at the project perimeter for the single-host / trusted-directory (home) shape; cross-actor integrity is a backend-substitution concern, not a filesystem-hardening one.

**Residual (tracked).** The perimeter invariant resolves-then-operates, leaving a theoretical time-of-check/time-of-use window (a co-located process swapping a checked path for a symlink between `resolve()` and `rename()`). Closing it requires `openat`/`O_NOFOLLOW` dir-fd traversal, which is the same pattern every sibling `Filesystem*Backend` needs — tracked as a framework-wide hardening effort (#480), not bolted onto this backend alone.

### Per-subdirectory symlink containment (security)

`_queue_root()` proves only that `queue/` itself is contained under `project_root`. The per-operation subdirectories — `queued/`, `claimed/`, `done/`, `dead-letter/`, and their `<role>` / `<lease_token>` namespaces — are themselves attacker-influenceable and MAY be symlinks. A symlinked `claimed/` (or any durable subdir) pointing outside `queue/` would let a `claim_next()`/`release()`/`move_to_dead_letter()` rename land OUTSIDE `project_root`, leaking work-item bytes (the `#426 _runs_root()` ancestor-escalation pattern). Every write AND read site (`claim_next`, `release`, `move_to_dead_letter`, `renew_lease`, `list_claimed`, `recover`, `export`) MUST resolve its target subdirectory and re-assert `is_relative_to(queue_root_resolved)` before use. In `FilesystemQueueBackend` the containment guard raises `PathTraversalError` internally and every operation catches it and fails SOFT: writes (`claim_next`, `release`, `move_to_dead_letter`, `renew_lease`) return `None` / no-op, and reads (`list_claimed`, `recover`, `export`) skip / return `[]`. This preserves the pre-carve `_cascade.py` contract, which had no containment check and so never propagated an exception to a caller. (A backend MAY instead fail-loud on writes; that is an implementation choice, not a Protocol requirement.) The containment check uses the RESOLVED anchor, but the path RETURNED for file operations is the UNRESOLVED `queue_root.joinpath(...)` — mirroring `FilesystemJournalBackend._journal_dir()` — so caller-visible `item.path` stays in the operator's own path representation (byte-identical to the pre-carve `_cascade.py` cron API; a project reached through a symlink does not get `item.path` silently rewritten to the real on-disk location). Because returned paths AND the `export()` relativization base (`self._project_root`) share that same unresolved representation, `relative_to()` never raises for the symlinked-`project_root` case, so the export is never silently emptied. `export()` ADDITIONALLY re-asserts per-leaf containment AND per-subdirectory containment: the export walk uses an explicit `iterdir()`-based recursive walk (not `rglob('*')`) that re-asserts `resolve() + is_relative_to(queue_root_resolved)` at each subdirectory descent and refuses to follow symlinked subdirs before descending (#477). This makes the no-follow behavior explicit and version-independent: as of CPython 3.13, `Path.rglob('*')` gained an opt-in `recurse_symlinks=True` parameter, but the *default* on every interpreter the framework supports (3.11–3.13) is `recurse_symlinks=False`, so default `rglob('*')` does NOT follow directory symlinks today. The explicit walk is therefore defense-in-depth — it does not depend on that stdlib default, adds a per-subdirectory `is_relative_to(queue_root_resolved)` containment re-assertion before each descent that `rglob` performs nowhere, and stays correct if a caller (or a future stdlib default) ever enables symlink recursion (e.g. a DoS via a symlinked subdir pointing at a large or cyclic tree). Because production `export()` never passes `recurse_symlinks=True`, the walk and default `rglob('*')` currently produce identical exported file sets on 3.11/3.12/3.13, so the symlinked-subdir hardening is verified by inspection + the per-subdir containment unit tests rather than by a strip-RED behavioral difference on today's interpreters; a 3.13+ CI lane that exercises the live `recurse_symlinks` vector is tracked in #595. Each durable work FILE is also resolved and checked `is_relative_to(queue_root_resolved)` before its bytes are read, so a symlinked work file pointing outside `queue/` cannot exfiltrate host bytes.
...
2. **Lease-expiry-recovery correctness.** `recover_stale_claims(backend, lease_seconds)` MUST correctly identify and recover items whose lease has expired and MUST NOT recover items with a valid future lease. The expiry signal is the `lease_expires_at` field returned by `list_claimed()`. Populating that field — including any mtime fallback for items lacking a sidecar — is the **backend's `list_claimed()` responsibility**, not the shared free function's: the filesystem backend's `_recover_stale_claims_native()` derives expiry from file mtime when no sidecar exists (mtime is filesystem-only and meaningless for Redis/SQS/DB), while the generic free-function path treats a `lease_expires_at` of `None` as stale (conservative recover-on-None), because a non-filesystem backend is expected to populate `lease_expires_at` from its own record. No shipping backend exercises the generic None=stale branch — `FilesystemQueueBackend` always routes through the native path.
...
13. **`enqueue()` producer primitive — atomic write to queued/<role>/item_name.** `enqueue(role, item_name, payload)` MUST atomically place `payload` bytes at `queued/<role>/<item_name>` using a temp-write-then-rename strategy (mkstemp → write → fsync(file) → rename → fsync(parent dir) → cleanup-on-failure) so no partial write is ever visible at the destination path and the renamed entry survives a power loss. `role` and `item_name` MUST each pass `_validate_bare_component`, which rejects path separators, null bytes, the empty string, the reserved names `.`/`..`, and any leading-dot name (reserving the dot-prefix namespace bucket-wide for internal producer temp files — see the "Producer/consumer temp-file isolation" addendum below) (it does NOT enforce a length cap), raising `PathTraversalError` before any I/O. `PathTraversalError` is an `AtomicAgentsError` subclass (NOT a `ValueError`). On rename failure, the implementation MUST clean up the temp file. The resulting file at `queued/<role>/<item_name>` MUST be claimable by `claim_next(role, ...)`. _(Conformance: TEST 57 — round-trip enqueue+claim; TEST 58 — atomicity via simulated rename failure; TEST 59 — path-separator strip-RED; TEST 60 — null-byte rejection raises PathTraversalError before any I/O; TEST 62 — leading-dot item_name rejection raises PathTraversalError before any I/O.)_

---

## Versioned normative addendum — #582 PR3 (2026-06-27): enqueue() producer primitive

MUST 13 (`enqueue()`) is the first producer primitive on the QueueBackend Protocol. Prior to PR3 the only way to place an item in `queued/<role>/` was to write the file directly (bypassing the Protocol), which was acceptable when the queue was SCAFFOLDING-ONLY. PR3 wires `enqueue()` in the conductor's conflict-queue **advisory** mechanism: when a conductor run detects an active gate conflict (another run's gate is awaiting a decision that overlaps this run's stage's conflict_keys), it calls `enqueue(role=<blocking_decision_id>, item_name=<safe_run_id>, payload=<advisory_bytes>)` on a per-agent QueueBackend to register an advisory record. The `role` is the **blocking gate's decision_id** (e.g. `gate-<hex>`), NOT a fixed literal; the `item_name` is the queued run's (sanitized) `conductor_run_id`.

**The advisory record is write-only / audit.** Nothing dequeues it — the conductor's serialization correctness comes from (a) the shared `conductor-conflict-scan` LockBackend lease that gates scan+suspend across runs and (b) the goal ledger (`held_conflict_keys` on the blocker's `awaiting_decision` sub-goal). The advisory queue is an audit breadcrumb and a demonstration of the producer primitive — it is explicitly NOT the authoritative waiter registry; the goal ledger is the source of truth for who-is-deferred-behind-whom. A deferred run **self-releases** off the ledger on its next `run()` invocation (it re-checks the blocking gate, not the queue). An orphaned advisory (gate answered before enqueue ran) is therefore harmless. The advisory is written ONCE per distinct (deferred-run, blocking-gate) pair: `item_name` is the stable deferred-run id, and a re-poll short-circuits at the conductor's top-of-run queued-event check (it returns the conductor `status='deferred'` again without reaching the enqueue site), so the bucket does NOT grow per poll. A consumer/drain/reap for the advisory bucket is out of scope for PR3 (the queue is not load-bearing for correctness); it is tracked in #662.

**SCAFFOLDING-ONLY designation retired.** `enqueue()` is a real, conformance-tested Protocol producer primitive (TEST 57–62) with a real caller (the conductor). The `_cascade.py` shim (cascaded stage transitions for model runs) remains un-wired to `QueueBackend` until #469; that use-case was always separate from the advisory queue.

**enqueue() containment.** `_validate_bare_component` rejects `role` and `item_name` values that contain path separators (`/`, `os.sep`), null bytes, the empty string, the reserved names `.`/`..`, or a leading dot, raising `PathTraversalError` before any I/O. Conductor run IDs are charset-validated (`[a-z0-9_-]`, the `crun-<uuid>` shape — `_mint_conductor_run_id()`), so they are already bare components; the conductor still defensively normalizes the `item_name` (replace any `:` with `-`, then a sha256-based fallback if validation still fails) to guard against a non-slug-safe id a custom caller might pass. The containment guarantee is: `queued/<role>/<item_name>` is always a single file directly under the role bucket, never a subdirectory or a traversal target.

**Producer/consumer temp-file isolation.** `enqueue()` stages its payload at `.{item_name}.<rand>.tmp` inside the SAME `queued/<role>/` bucket before the atomic rename to the final name. The reference `claim_next()` therefore MUST skip candidate files matching the producer's dot-prefixed `.tmp` pattern (`name.startswith(".") and name.endswith(".tmp")`) so neither (a) a `claim_next()` racing a concurrent `enqueue()` nor (b) a crash-leaked orphan temp from a producer that died between `mkstemp` and `rename` is ever promoted into work. A valid `item_name` can never begin with `.` because `_validate_bare_component` ENFORCES the leading-dot rejection (it rejects any value starting with `.`, not merely the bare `.`/`..` names), so the skip provably cannot exclude a legitimate item. This preserves the atomic-producer round-trip the Protocol implies (Principle #8: crashes leave recoverable artifacts, not corruption). _(Conformance: TEST 61 — an orphan `.foo.<rand>.tmp` left in `queued/<role>/` is NOT returned by `claim_next`; TEST 62 — a leading-dot `item_name` is rejected at `enqueue()` so the dot-prefix namespace stays reserved.)_
