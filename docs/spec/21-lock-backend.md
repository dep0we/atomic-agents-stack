...
PR 1 (this PR) is pure scaffolding. The Protocol exists; the filesystem reference impl exists; tests + spec exist. **No call site has been migrated.** ``_locks.AgentLock`` is unchanged. ``agent.py:2073``, ``dream.py:1098``, ``memory/filesystem.py:506`` and the inline ``_fcntl.flock`` at ``memory/filesystem.py:629-639`` all continue to use the legacy primitive. PR 2 of the arc does the wiring + deprecation shim.
