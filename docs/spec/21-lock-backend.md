...
PR 1 shipped pure scaffolding — Protocol, filesystem reference impl, tests, spec. PR 2 wired the four legacy ``AgentLock`` / ``_DreamLock`` call sites plus ``doctor.check_locks`` through the backend and converted ``_locks.py`` to a deprecation shim (sunset planned for v1.1 (deferred from v1.0 per #201 PR 5 release decision) per CLAUDE.md rule #14). The inline ``_fcntl.flock`` at ``memory/filesystem.py``'s ``_per_file_lock`` is **deliberately NOT subsumed** (filesystem-implementation invariant — see below). PR 3 ships the distributed reference impl + operator override surface. PR 4 locks this spec and parametrizes the conformance suite across both backends.
