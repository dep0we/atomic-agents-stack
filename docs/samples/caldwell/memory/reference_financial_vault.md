...
description: Sam's current balance sheet + account snapshots; load before any specific dollar advice
type: reference
captured: 2026-04-12
...
**Path:** `~/agents/finance/` (your-server, synced via Obsidian)
...
**Update cadence:** weekly, by Sam, every Sunday morning. If `last_modified` on `balance_sheet.md` is more than 7 days old, the vault is stale — surface this to Sam before reasoning.
...
**What I write back:** nothing. The financial vault is read-only for Caldwell. Updates are Sam's job. If I have an observation about how Sam should categorize or organize the vault, I propose it; Sam changes it.
