---
schema_version: 1
name: Freelance editing retainers — two clients stable through 2026
description: Two retainer clients; treat as stable income through year-end
type: project
captured: 2026-04-22
last_seen: 2026-05-01
sources:
  - conversation_2026-04-22
confidence: high
pinned: false
expires_at: 2026-12-31
supersedes: null
superseded_by: null
tags: [income, retainer, stable]
---

**Status (as of 2026-05-01):** Both retainer relationships stable. No renewal risk flagged for 2026.

**Clients:**
- ClientCo A — example client, monthly retainer, ongoing
- ClientCo B — example client, monthly retainer, ongoing

**For income planning purposes:**
- Treat the combined retainer revenue as a fixed, predictable component of household income through Dec 31, 2026
- Reference the actual dollar amount in `~/agents/finance/income/` (do not state amount in this memory; load it live)
- Do NOT factor in freelance new-client wins — those are opportunistic upside, not baseline (see `project_freelance_editing_growth.md`)

**Risk to surface if it changes:**
- Either client signaling end-of-engagement → re-plan Q3/Q4 immediately
- Retainer scope reduction → recompute baseline income before the next paycheck

**How to apply:**
- For Q3 income planning, count this as the confirmed-revenue component
- For debt payoff pace projections, treat as stable inflow through year-end
- For "should I take on this expense" questions, retainer revenue defines the steady-state cushion

**Expires:** 2026-12-31. Re-confirm at year-end before extending the assumption into 2027.
