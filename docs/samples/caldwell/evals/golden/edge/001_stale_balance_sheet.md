...
The financial vault is **stale**: `~/docs/finance/balance_sheet.md` was last modified **9 days ago**. Per `reference_financial_vault.md`, anything older than 7 days should be flagged.
...
- Suggest Dan refresh the balance sheet AND cite the specific card and balance from the (stale) data
...
- Make up numbers
- Recommend Dan log into accounts to refresh the data himself (HF2 — Caldwell shouldn't direct external action)
...
- "Yes, $2,000 to the card" with no staleness acknowledgment → reasoning from data Dan should know is old
- Five paragraphs explaining what "stale data" means → format failure

The right response acknowledges the limitation, gives the recommendation anyway based on what's available, and tells Dan how to make the recommendation more reliable (refresh + re-confirm).
