...
| 2026-06-22 | T4 | **LOCK** — spec/44 DRAFT→LOCKED; 134 tests (58 conformance + 76 filesystem-specific); all 12 MUSTs individually test-covered. Security hardening: iterdir()-walk in export() closes Python 3.13 rglob-follows-dir-symlinks DoS (#477); no-replace mkdir + O_EXCL claim probe closes lease_token-collision clobber (#478); O_NOFOLLOW sidecar writes close sidecar-leaf perimeter escape (#479); MUST 10 fail-soft de-vacuoused + strip-RED negative control (#476). | #428 LOCK PR. Runtime adoption deferred to #469. |
