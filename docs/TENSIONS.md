...
**Update (2026-07, Phase 2b, #742):** the core↔fleet seam now reads rates through a function, not the dict. `atomic_agents.core_api.get_model_rates(model_id)` returns a copy of a model's `input`/`output` rates (or `None` for an unknown model); `PRICING` itself stays private to `_costs.py`. This doesn't touch the rate-staleness problem this entry is about — the table is still hardcoded and still needs a PR to update — but it closes off the *other* risk the table carried: `advisor/` and `dashboard/` used to read `PRICING` as a raw dict at three call sites, so a future rename or reshape of the table (say, to load it from a file) would have broken every caller silently. Now a price-table edit can happen without touching the fleet code at all.
...
- **Spec-numbering policy** — `docs/spec/` numbers are reserved for agent + backend contracts; extension specs live with the extension. At the physical move the fleet specs (52-57) are **renumbered** to a fresh fleet-local scheme so core's sequence carries no gap (Phase 2c ruling, below). (Relates to T9.)
- **`core_api` surface (shipped Phase 2b, #742)** — `atomic_agents/core_api.py` publishes exactly six names (`atomic_write`, `safe_resolve_under`, `get_agents_root`, `parse_model_md`, `calc_cost`, `get_model_rates`) as the internal-stable core↔fleet contract, versioned together with core, not a broadly-advertised public API. `advisor/`, `dashboard/`, and `manage/` used to reach into core's private modules (`_io`, `_costs`, `_platform`, `_model`) directly — the correct direction (extension → core) but through an unstable seam. A static AST-based guard test (`tests/test_core_api.py`) now asserts none of the three extension packages import a core-private module by any dotted-path shape (absolute, aliased, relative), matching exact segments rather than a substring — a substring match on `_model` would false-positive on a file like `manage/set_model.py`. The extension-owned *test* files still reached into core privates directly (e.g. mutating `_costs.PRICING` in fixtures) — that half closed in **Phase 2c (#743/#748)**: those test sites repointed onto `core_api.get_model_rates()`, and the AST guard was extended to scan extension-owned test files (residual guard-completion tracked in #747).

**Extraction conventions (ruled Phase 2c, #743 — materialize at the physical Fair Copy move, not yet):** the in-repo test-seam half of Phase 2c shipped in #743/#748 (extension-owned test files repointed off core privates; the AST guard extended to scan test files; residual guard-completion in #747). The remaining Phase 2c items are *conventions* for the eventual physical split, recorded here now and executed at the move:
- **Fleet spec home + classification** — specs 52 (fleet-console), 53 (scoring), 54 (recommendations), 55 (fleet-management CLI), 56 (fleet-monitor), 57 (agent-detail) move to `fair-copy-fleet`; specs 50 (conductor, orchestration) and 51 (AgentRegistryBackend, a backend protocol) stay in core.
- **Fleet spec numbering** — the moved specs are **renumbered** to a fresh fleet-local scheme (e.g. `fleet/01..06`) at the move, while still DRAFT, so core's `docs/spec/` sequence carries no 52-57 gap; existing bare `spec/52`-style citations are updated to the new fleet-local names as part of the move. (Maintainer-ruled 2026-07-18, over keep-the-numbers-with-a-gap.)
- **Cross-repo spec citations** — within a repo, keep the bare `spec/NN` shorthand; *across* the boundary, use an explicit origin prefix in prose (`core spec/22`, `fleet spec/54`) plus a version-tag-pinned URL/anchor, never a bare number.
- **Two CHANGELOGs** — after the split each repo owns its own CHANGELOG (source of truth for its SemVer line); a change spanning both records its own half in each plus a cross-reference (companion PR/issue link) matching the dependency arrow. Convention, not a CI gate.
- **Pre-split history** — the new fleet CHANGELOG opens with a short provenance note pinning the extraction point (extracted from `atomic-agents-stack` at the v3.0.0 rename) rather than copying core-side entries (the console arcs shipped as core PRs).
- **Fleet versioning** — `fair-copy-fleet` versions **independently** (not lockstep, not v3.0.0) with a declared core-major dependency floor (e.g. `fair-copy>=3.0,<4.0`), born fresh at **v0.1.0** (pre-1.0 — room to reshape the new package without a breaking-change label). (Maintainer-ruled 2026-07-18, over v1.0.0.)
...
| 2026-07-17 | T17 / T7 | **Added** — the `core_api` surface shipped (Phase 2b): `atomic_agents/core_api.py` re-exports six names (`atomic_write`, `safe_resolve_under`, `get_agents_root`, `parse_model_md`, `calc_cost`, `get_model_rates`) as the internal-stable core↔fleet contract; `_costs.get_model_rates()` is the new T7 accessor (`PRICING` stays private); the 16 borrow sites in `advisor/`, `dashboard/`, `manage/` are repointed off core-private modules onto `core_api`; a static AST-based guard test (`tests/test_core_api.py`) makes the repoint permanent. | #742 (Phase 2b of the core/extension split). Closes the *source*-layer half of the extension→core-private seam (test fixtures still borrow `_costs.PRICING` directly — that's Phase 2c). `deploy/` intentionally NOT repointed (it's core; routing core through its own core↔fleet contract module is incoherent). Guard test strip-RED verified (re-added a stripped private import, confirmed the guard fails, reverted) and confirmed no false-positive on a `set_model.py`-shaped filename (the substring-vs-segment distinction the AST approach exists to get right). |
| 2026-07-18 | T17 | **Added** — Phase 2c: the extension-test half of the seam closed (#743/#748) + the extraction conventions ruled and recorded (spec home/renumber, cross-repo citation format, two CHANGELOGs, pre-split history, independent v0.1.0 fleet versioning). | #743 (Phase 2c). The in-repo test code shipped in #748: the two `tests/advisor/` `_costs.PRICING` read sites repointed onto `core_api.get_model_rates()`; the injection strip-RED test rewritten from mutating the private dict to monkeypatching the extension-owned `advisor.score.get_model_rates` binding (exact-tie `<`/`<=` boundary preserved); the AST seam-guard extended to scan extension-owned test files. The convention rulings (this docs change) materialize at the physical Fair Copy move. Maintainer-ruled the two open forks (renumber fresh; born v0.1.0). Residual guard-completion (relative dynamic-import strings + string-`patch()` targets) tracked in #747. |
