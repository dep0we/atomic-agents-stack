...
Atomic Agents is a vault-native AI agent framework: agents live as plain markdown files, the runtime is stateless, and storage is moving toward swappable protocols layer by layer. **Twelve backend protocols shipped through v1.0; twenty-one total as of v2.0.0** — see `docs/protocols-shipped.md` for the per-protocol summary (reference impls, capabilities, operator overrides, doctor checks, Implementer Contracts, and what cliff each closes).
...
```
       ...
       Journal              Delegation (tree-cap) OTel trace export 🟡 (#341 PR1: call span)
                  ...
                  GoalBackend ✅ (#425 PR 1 + #448 PR1/PR2/PR3 arc-closer + #483 PR1 clock-injection/shim/agent_root + #496 PR1 backend-universe alignment, LOCKED spec/41 — the 14th, v2.0.0)
                  OutcomeBackend ✅ (#426 PR 1 + #448 PR2 write-path, LOCKED spec/42 — the 15th, v2.0.0)
                  JournalBackend 🟡 (#427 PR 1, DRAFT spec/43 — the 16th, v2.0.0)
                  QueueBackend 🟡 (#428 PR 1, DRAFT spec/44 — the 17th, v2.0.0)
                  IdempotencyBackend ✅ (#520 PR 1 + PR 2 arc-closer, LOCKED spec/45 — the 18th, v2.0.0)
                  EmbeddingBackend 🟡 (#200 PR 2 + #544 PR1 + #544 PR2a, DRAFT spec/46 — the 19th, v2.0.0)
                  ConversationBackend 🟡 (#535 PR 1, DRAFT spec/47 — the 20th, v2.0.0)
                  PrincipalBackend 🟡 (#556 PR 1, DRAFT spec/48 — the 21st, v2.0.0)
                                │
...
Run `uv run pytest --collect-only -q | tail -1` for the live test count (last refresh: 5,581 tests collected, 2026-06-21). New backend protocols add ~25 conformance + ~10 impl-specific tests. New features ship with tests. Migration-shaped PRs need parameterized fixture tests across the backend protocol — the conformance suite is what keeps the protocol honest.
