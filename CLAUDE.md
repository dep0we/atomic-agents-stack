...
**v0.13.0, alpha, PUBLIC.** Core runtime stable, 1953 tests passing (+ 34 conformance skips: 8 capability-gated ToolRegistry skips — `supports_install` / `supports_uninstall` / `supports_versioning` / `supports_skills_catalog` tests skip on whichever reference backend declares False — plus the 9 capability-gated AgentProfile skips and the prior 7 conformance skips: 5 cross-process Redis tests that lift against real Redis instead of fakeredis + the 2 pre-existing LLM-only skips on PolicyJudge) on Python 3.11/3.12. **Seven backend protocols shipped**:
