...
```
       ...
       Journal              Delegation (tree-cap)
                  ...
                  Persona 🟡  AgentProfile 🟡  ToolRegistry 🟡  Corpus 🟡
                                │
...
**v0.13.0, alpha, PUBLIC.** Core runtime stable, 1750 tests passing (+ 16 conformance skips: 7 capability-gated AgentProfile skips — skill-content tests skip on SQLite, filesystem-shape tests skip on SQLite — plus the prior 7 conformance skips: 5 for cross-process Redis tests that lift against real Redis instead of fakeredis + the 2 pre-existing LLM-only skips on PolicyJudge) on Python 3.11/3.12. **Five backend protocols shipped** (`AgentProfileBackend` arc #63 is 3-of-4 PRs landed — locking in PR 4):
...
MCP client support shipped (PRs #55 + #56). **AgentProfileBackend arc (#63) is 3 of 4 PRs landed (unreleased)**: `FilesystemAgentProfileBackend` + `SQLiteAgentProfileBackend` reference impls, JSON-based snapshot trio implementation across both backends, parametrized conformance suite, `supports_skills` capability dimension added to `ProfileCapabilities`, operator override via `ATOMIC_AGENTS_PROFILE_BACKEND=sqlite` (+ optional `ATOMIC_AGENTS_PROFILE_BACKEND_URL`) — single-host operators get a working SQLite default at `<scope_root>/.profile.db` by flipping ONE env var. PR 4 locks spec/24 + adds the §"Implementer contract for registry-backed backends" section (Postgres / git / SaaS-API future adapters mirror the SQLite reference's shape). Active backlog covers the remaining protocols (Persona / ToolRegistry / Corpus / Policy). Single-developer project; reference implementation that anyone can use, fork, or extend.
