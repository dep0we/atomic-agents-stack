...
```
       ...
       Journal              Delegation (tree-cap) OTel trace export 🟡 (#341 PR1: call span)
                  ...
                  QueueBackend ✅ (#428 PR 1 + LOCK PR, LOCKED spec/44 — the 17th, v2.0.0)
...
| `docs/spec/01-...49-deploy.md` | Locked spec (48 docs today, 40 locked + 8 DRAFTs/RFCs at spec/26 (cascade bundle), spec/30 (responsibility audit), spec/37 (serve), spec/39 (otel-export), spec/43 (journal-backend), spec/47 (conversation-backend), spec/48 (principal-backend), and spec/49 (deploy)). The product. |
