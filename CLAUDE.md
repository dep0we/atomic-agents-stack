...
Run `uv run pytest --collect-only -q | tail -1` for the live test count (last refresh: 4,505 tests collected, 2026-06-14). New backend protocols add ~25 conformance + ~10 impl-specific tests. New features ship with tests. Migration-shaped PRs need parameterized fixture tests across the backend protocol — the conformance suite is what keeps the protocol honest.
...
| `docs/spec/01-...44-queue-backend.md` | Locked spec (43 docs today, 36 locked + 7 DRAFTs/RFCs at spec/26 (cascade bundle), spec/30 (responsibility audit), spec/35 (init wizard), spec/37 (serve), spec/39 (otel-export), spec/43 (journal-backend), and spec/44 (queue-backend)). The product. |
