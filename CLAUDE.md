...
**v0.13.0, alpha, PUBLIC.** Core runtime stable, 1750 tests passing (+ 16 conformance skips: 9 capability-gated AgentProfile skips — skill-content tests skip on SQLite, filesystem-shape tests skip on SQLite, plus the 2 path-traversal refusal tests gated on `supports_snapshot` per backend — plus the prior 7 conformance skips: 5 for cross-process Redis tests that lift against real Redis instead of fakeredis + the 2 pre-existing LLM-only skips on PolicyJudge) on Python 3.11/3.12. **Six backend protocols shipped**:
...
MCP client support shipped (PRs #55 + #56). Active backlog covers the remaining protocols (Persona / ToolRegistry / Corpus / Policy). Single-developer project; reference implementation that anyone can use, fork, or extend.
