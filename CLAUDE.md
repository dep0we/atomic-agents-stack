...
Run `uv run pytest --collect-only -q | tail -1` for the live test count (last refresh: 5,735 tests collected, 2026-06-24). New backend protocols add ~25 conformance + ~10 impl-specific tests. New features ship with tests. Migration-shaped PRs need parameterized fixture tests across the backend protocol — the conformance suite is what keeps the protocol honest.
...
| `docs/spec/01-...51-agent-registry-backend.md` | Locked spec (49 docs today, 43 locked + 6 DRAFTs/RFCs at spec/26 (cascade bundle), spec/30 (responsibility audit), spec/37 (serve), spec/39 (otel-export), spec/49 (deploy), and spec/51 (agent-registry-backend)). The product. |
