...
```
       ...
       Journal              Delegation (tree-cap) OTel trace export 🟡 (#341 PR1: call span)
                  ...
                  EmbeddingBackend 🟡 (#200 PR 2, DRAFT spec/46 — the 19th, v1.5)
...
Run `uv run pytest --collect-only -q | tail -1` for the live test count (last refresh: 5,415 tests collected, 2026-06-19). New backend protocols add ~25 conformance + ~10 impl-specific tests. New features ship with tests. Migration-shaped PRs need parameterized fixture tests across the backend protocol — the conformance suite is what keeps the protocol honest.
...
| 19 | EmbeddingBackend | #200 PR 2 (DRAFT spec/46) | OpenAI (OpenAIEmbeddingBackend) |
