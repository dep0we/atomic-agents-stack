...
3. **Builds serialize.** Only ONE `arc-execute`/`arc-finish` at a time — they share one working tree. Never fire two concurrently; queue the second. `arc-discovery` is read-only and may run alongside a build or in parallel with other discoveries.
