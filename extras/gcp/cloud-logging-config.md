...
- **FilesystemLogBackend** (v0 default): JSONL files on the persistent disk at
  `<agent_root>/log/`. Query by reading/grepping the JSONL or via the dashboard
  (see §Querying the JSONL audit trail).
- **PostgresLogBackend** (when issue #258 ships): Cloud SQL. Query via psql
  against `run_records`, or the dashboard pointing at
...
**FilesystemLogBackend (v0):** read or grep the JSONL files on the persistent
disk. Each day's runs land at `<agent_root>/log/YYYY-MM/YYYY-MM-DD.jsonl`, one
