...
# ──────────────────────────────────────────────────────────────────
# T5 (instance-identity / boundary 1): the coordinator passes the SAME backend
# objects from the gate agent into the OutcomeRunner constructor — first half of
# the backend-universe alignment property (#496 PR1).
#
# The coordinator reads agent.log_backend/policy_backend/profile_backend and
# threads them into OutcomeRunner(...) as keyword args. This test asserts that
# the EXACT same object references land in the OutcomeRunner constructor call
# (not copies, not None, not auto-generated MagicMock attrs).
#
# Two boundaries make up the full chain. This test covers BOUNDARY 1
# (gate agent → OutcomeRunner ctor kwargs). BOUNDARY 2 (OutcomeRunner kwargs →
# internal AtomicAgent ctor) is covered by
# tests/test_outcome.py::test_outcome_runner_forwards_backends_to_internal_agent.
# Together they verify the gate agent's resolved backends ARE the ones the
# runner's internal AtomicAgent spends through ("gate backend == spend backend").
#
# Implementation note: all tests patch 'atomic_agents.outcome.OutcomeRunner'
# with a MagicMock. The mock instance's attributes are auto-generated by
# MagicMock.__getattr__ and do NOT reflect what was passed into the constructor.
# The correct assertion target is MockRunner.call_args.kwargs — the kwargs dict
# the coordinator passed to OutcomeRunner(...). Asserting on the mock instance's
# _log_backend/_policy_backend/_profile_backend would always pass regardless
# of what the coordinator actually threaded (false pass).


def test_coordinator_threads_gate_agent_backends_into_runner(agent_with_goal):
    """OutcomeRunner must receive the SAME log_backend/policy_backend/profile_backend
    instances as the gate agent — boundary 1 of the backend-universe alignment
    property (spec/41 §'Goal-outcome composition', #496 PR1).

    Asserts on MockRunner.call_args.kwargs to inspect what the coordinator actually
    passed to OutcomeRunner(...) — NOT the mock instance's auto-generated attrs.
    This is the only correct way to verify the coordinator threads the right objects.
    Boundary 2 (runner kwargs → internal AtomicAgent ctor) is asserted by
    tests/test_outcome.py::test_outcome_runner_forwards_backends_to_internal_agent.
    """
    agents_root, agent_name, agent_root = agent_with_goal
    gm = _make_gm(agent_with_goal)
    ...
    # Build a gate agent mock with EXPLICIT sentinel backend objects.
    # Using object() sentinels (not MagicMock children) so identity assertions
    # are unambiguous — a sentinel can only be IS itself.
    sentinel_log = object()
    sentinel_policy = object()
    sentinel_profile = object()

    agent = _make_agent_mock(allow=True)
    agent.log_backend = sentinel_log
    agent.policy_backend = sentinel_policy
    agent.profile_backend = sentinel_profile
    ...
    # The coordinator must have called OutcomeRunner(...) with the sentinel backends
    # as keyword args. call_args.kwargs is what was actually passed.
    ctor_kwargs = MockRunner.call_args.kwargs
    assert ctor_kwargs["log_backend"] is sentinel_log, (
        "OutcomeRunner must receive the gate agent's log_backend (same object). "
        f"Got: {ctor_kwargs.get('log_backend')!r}, expected: {sentinel_log!r}"
    )
    assert ctor_kwargs["policy_backend"] is sentinel_policy, (
        "OutcomeRunner must receive the gate agent's policy_backend (same object). "
        f"Got: {ctor_kwargs.get('policy_backend')!r}, expected: {sentinel_policy!r}"
    )
    assert ctor_kwargs["profile_backend"] is sentinel_profile, (
        "OutcomeRunner must receive the gate agent's profile_backend (same object). "
        f"Got: {ctor_kwargs.get('profile_backend')!r}, expected: {sentinel_profile!r}"
    )
...
# custom-backend caller changes).
#
# This test uses a different gate-agent construction pattern from T5:
# the agent mock carries a custom MagicMock log_backend (simulating an operator
# who pinned a custom LogBackend on their AtomicAgent). The behavioral property
# is that the coordinator forwards it to OutcomeRunner — if it didn't, OutcomeRunner
# would independently resolve a filesystem backend, bypassing the operator's backend.
#
# Scope (honest): with OutcomeRunner patched, this asserts the coordinator→runner
# boundary (boundary 1) only — that the custom backend is FORWARDED, not that a run
# actually spends/records through it. The boundary-2 forwarding (runner kwarg →
# internal AtomicAgent ctor) is asserted by
# tests/test_outcome.py::test_outcome_runner_forwards_backends_to_internal_agent;
# the runner STORAGE of the kwarg is asserted by
# tests/test_log_integration.py::test_outcome_runner_kwarg_threaded_to_internal_agent
# and tests/test_profile_integration.py::test_outcome_runner_threads_profile_backend.


def test_coordinator_forwards_custom_log_backend_to_runner(agent_with_goal):
    """A custom log_backend on the gate agent must be FORWARDED to OutcomeRunner
    (coordinator→runner boundary) — the custom-backend programmatic path the
    backend-universe alignment property serves (#496 PR1, spec/41 §'Goal-outcome
    composition').

    An operator who injects a custom LogBackend into the gate agent's AtomicAgent
    expects the OutcomeRunner's internal AtomicAgent to ultimately use the SAME
    store for cost reads and log writes. This test proves the FORWARDING at the
    coordinator boundary (call_args.kwargs); it does NOT itself run/spend (OutcomeRunner
    is mocked). The end of the chain — that the run's internal AtomicAgent is built
    with this backend — is proved by
    tests/test_outcome.py::test_outcome_runner_forwards_backends_to_internal_agent.
    """
    agents_root, agent_name, agent_root = agent_with_goal
    gm = _make_gm(agent_with_goal)
    ...
    # Simulate an operator who pinned a custom LogBackend on their gate agent.
    # MagicMock with a name makes assertion messages readable.
    custom_log_backend = MagicMock(name="custom_log_backend")

    agent = _make_agent_mock(allow=True)
    agent.log_backend = custom_log_backend
    # policy_backend and profile_backend stay as auto-generated MagicMock attrs
    # (the behavioral test focuses on log_backend; the instance-identity test
    # above covers all three together with sentinel objects).
    ...
    ctor_kwargs = MockRunner.call_args.kwargs
    assert ctor_kwargs["log_backend"] is custom_log_backend, (
        "A custom log_backend injected into the gate agent must be forwarded to "
        "OutcomeRunner(...) as a keyword arg so the runner's internal AtomicAgent "
        "uses the same store for cost reads and log writes as the pre-dispatch gate. "
        f"Got: {ctor_kwargs.get('log_backend')!r}, expected: {custom_log_backend!r}\n"
        "NOTE: this verifies the coordinator→runner boundary only; the deeper "
        "runner→AtomicAgent forwarding is asserted by "
        "test_outcome.py::test_outcome_runner_forwards_backends_to_internal_agent."
    )
