...
- [44: QueueBackend Protocol](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/spec/44-queue-backend.md): cascade work-queue protocol; `FilesystemQueueBackend` reference impl; POSIX-rename atomic claim; `QueueExport` wired into spec/40 Exportable; closes TENSIONS T4 (single-host claim atomicity); SCAFFOLDING-ONLY carve; LOCK hardening: iterdir()-walk in `export()` (#477), no-replace mkdir + O_EXCL claim probe (#478), `_write_no_follow` O_NOFOLLOW sidecar writes (#479), MUST 10 fail-soft de-vacuoused (#476); 137 tests, 12 MUSTs; runtime adoption deferred to #469; **LOCKED** (#428)
...
- [50: Conductor](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/spec/50-conductor.md): durable, resumable multi-stage playbook orchestration; PLAYBOOK.md loader + schema; sequential automated stages; goal-ledger resume cursor; run-cap tree-cap; gate suspension + `resume()` + `GateDecision` artifact; `awaiting_decision`/`skipped` statuses; spec/41 RE-LOCKed; DRAFT (#575 design, #580 PR1, #581 PR2)
...
| `QueueBackend` | ✅ Shipped (LOCKED spec/44) | Cascade work-queue protocol; `FilesystemQueueBackend` reference impl; POSIX-rename atomic claim; SCAFFOLDING-ONLY carve of `_cascade.py` queue cluster; `QueueExport` wired into `Exportable` (spec/40); closes TENSIONS T4 (single-host claim atomicity cliff); 137 tests; runtime adoption deferred to #469; the seventeenth backend Protocol | [`spec/44`](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/spec/44-queue-backend.md) |
...
**v2.0.0, stable.** Core runtime stable. Twelve of twelve backend protocols shipped and locked at v1.0 (see the backend protocols table above); `MCPServerRegistryBackend` LOCKED at PR 5 v1.0. A thirteenth, `SecretBackend` (#340), shipped in v2.0.0 (both PRs merged, spec/38 LOCKED, Filesystem + GCP Secret Manager reference impls). A fourteenth, `GoalBackend` (#425 + #448 PR1/PR2/PR3 + #483 PR1 + #496 PR1 + #642 PR1 RE-LOCK), shipped in v2.0.0 (LOCKED spec/41 — RE-LOCKed at 13 MUSTs in #642 PR1, `FilesystemGoalBackend` reference impl, write-path adoption: `GoalManager` load/save/history routed through backend, `AtomicAgent.goal_backend` live-wired, archive data-loss fix; goal-outcome coordinator `dispatch_sub_goal_as_outcome()` with fail-closed cost gate + CAS guard — arc-closer #448 PR3; clock injection + `GoalManager.archive()` thin shim + `agent_root` resolved at init — cleanup #483 PR1; backend-universe alignment: coordinator threads gate agent's `log_backend`/`policy_backend`/`profile_backend` into `OutcomeRunner` — #496 PR1; multi-goal addressing: `create_goal()`/`list_goals()` on GoalBackend Protocol, `AddressableGoalBackend` Protocol + `for_goal()` scope handle, `GoalCapabilities.supports_multi_goal`, `GoalAlreadyExists` exception, `export()` fail-loud guard — #642 PR1). A fifteenth, `OutcomeBackend` (#426 + #448 PR2), shipped in v2.0.0 (LOCKED spec/42, `FilesystemOutcomeBackend` reference impl, write-path adopted: `OutcomeRunner.run()` routes through the backend, custom-`output_dir` `result.json` relocated to the canonical path). A sixteenth, `JournalBackend` (#427), shipped in v2.0.0 PR 1 + LOCK PR (LOCKED spec/43, `FilesystemJournalBackend` reference impl, ADOPT-NOW wiring). A seventeenth, `QueueBackend` (#428), shipped in v2.0.0 PR 1 + LOCK PR (LOCKED spec/44, `FilesystemQueueBackend` reference impl, SCAFFOLDING-ONLY carve from `_cascade.py`, closes TENSIONS T4). An eighteenth, `IdempotencyBackend` (#520), shipped in v2.0.0 (PR 1 + PR 2 arc-closer) (LOCKED spec/45, `FilesystemDedupLedger` reference impl — `begin()`/`commit()`/`lookup()`/`release_lease()` Protocol + O_EXCL atomic lease claim + MARKER-ONLY terminal entry; PR 2 wired the two-phase dedup gate into `agent.call(idempotency_key=...)` with serve/queue/cron triggers, RunRecord audit fields, and the spec/22 versioned normative addendum). A nineteenth, `EmbeddingBackend` (#200 PR 2 of 3 + #544 PR1/PR2a/PR2), shipped in v2.0.0 (LOCKED spec/46, `OpenAIEmbeddingBackend` reference impl — pluggable embedding generation for semantic recall; MUST-NOT-RAISE None-fallback + `len(out)==len(in)`; `EMBEDDING_PRICING` isolated from chat `PRICING`; dimension honesty at construction + produced vector; SecretBackend key resolution; registry + pgvector wiring + `input_type` kwarg shipped PR3 (#200); #544 PR1: batch embed cost gate at `agent.call()` capture-commit site + `PRIMITIVE_EMBED` + `check_embedding_backend()` doctor check + spec/20/22/34 normative addenda; #544 PR2a: dedicated `embed_cost` JSONL record (`cost_usd=actual_usd` visible to `sum_cost_for_period` across calls) + merge-write pre-read reservation (sized from preserved target body, Principle #4) + spec/22 versioned normative addendum; #544 PR2: CLI corpus-query embed gate + 4 gate-site normative MUSTs + direct-caller gate boundary (#586) + pgvector conformance fold + DRAFT→LOCKED). A twentieth, `ConversationBackend` (#535 PR 1 + LOCK PR), shipped in v2.0.0 (LOCKED spec/47, `FilesystemConversationBackend` reference impl; `Principal` typed identity key + `LOCAL_PRINCIPAL` home-user default; two-part cross-principal Guard (2); `agent.call(conversation_id=...)` wiring; model-aware token-budget derivation off `LLMCapabilities.max_input_tokens`; `check_conversation_backend()` doctor dual-probe; #553 write-back reorder fix; SQLite + Postgres v2→v3 migration; 149 tests). A twenty-first, `PrincipalBackend` (#556 PR 1 + LOCK PR), shipped in v2.0.0 (LOCKED spec/48, `LocalPrincipalBackend` + `StaticClaimsPrincipalBackend` reference impls; HARD-REFUSE gate placed BEFORE idempotency dedup lookup; serve HYBRID flow fail-closed opt-in; doctor wired into `run_doctor()` with credential-redaction; full MUST-coverage conformance suite). A twenty-second, `AgentRegistryBackend` (#607 PR 1), shipped in v2.0.0 (DRAFT spec/51, `FilesystemAgentRegistryBackend` reference impl; fleet-agent discovery by `model.md`-present predicate; `governance.md` typed schema; FULL ADOPT-NOW: `discover_agents()` rewired through registry, doctor check wired with bidirectional reconcile, `atomic-agents init` writes governance stub). The v1.0 Protocol surface is stable per SemVer Major. See [`docs/deployment/versioning.md`](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/deployment/versioning.md) for the breaking-change policy. Single-maintainer project; reference implementation anyone can use, fork, or extend.
