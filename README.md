...
| `LockBackend` | 🟡 In progress — Protocol scaffolding + `FilesystemLockBackend` reference impl + conformance suite shipped at [#60](https://github.com/dep0we/atomic-agents-stack/issues/60) PR 1 (unreleased). No behavior change — `_locks.AgentLock` is unchanged; PR 2 wires the backend into the four legacy lock sites + `doctor.check_locks`; PR 3 ships a distributed reference impl; PR 4 locks `docs/spec/21-lock-backend.md` and parameterizes conformance across both backends. | [`spec/21-lock-backend.md`](docs/spec/21-lock-backend.md) (DRAFT — locks at PR 4) |
...
tests/                          # 1327 tests, all passing on Python 3.11 + 3.12
