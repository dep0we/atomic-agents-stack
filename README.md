...
That's what `atomic-agents-stack` defines, in locked spec docs (plus active RFCs), with a Python reference implementation, 4,160+ tests, and a Caldwell sample shipping 5 days of real JSONL run logs, a rendered cost dashboard, evals across happy / edge / adversarial / decline categories, and a helper-pattern day showing ~76% cost savings vs. all-Opus.
...
| **Numbered, locked spec** | 34 locked docs in `docs/spec/` (+ 7 DRAFTs/RFCs in progress) | API + concept docs | API + concept docs | API reference + concept docs | None |
...
- **Spec-level file layout.** 34 numbered docs lock the contract (plus 7 DRAFTs/RFCs in progress); conformance is testable; alternate implementations are possible.
...
- [20: Memory backend protocol](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/spec/20-memory-backend.md): the protocol-pattern moat; `ATOMIC_AGENTS_MEMORY_BACKEND` env var + `memory_backend=` kwarg + `get_default_memory_backend` factory + operator override surface (#382)
...
- [41: GoalBackend Protocol](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/spec/41-goal-backend.md): goal state persistence abstraction; `FilesystemGoalBackend` reference impl; `apply_transition()` atomic primitive; `GoalExport` wired into spec/40 Exportable; DRAFT (#425)
- [42: OutcomeBackend Protocol](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/spec/42-outcome-backend.md): outcome state persistence abstraction; `FilesystemOutcomeBackend` reference impl; `result.json` byte-identical on-disk + portable `export()` with relative artifact refs (Option C); `OutcomeExport` wired into spec/40 Exportable; 9-MUST Implementer Contract (write-once/result-immutability as the unique 9th); DRAFT (#426)
...
| `GoalBackend` | ✅ Shipped (DRAFT spec/41) | Goal state persistence; `FilesystemGoalBackend` reference impl; `apply_transition()` atomic primitive; `GoalExport` wired into `Exportable` (spec/40); the fourteenth backend Protocol | [`spec/41`](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/spec/41-goal-backend.md) |
| `OutcomeBackend` | ✅ Shipped (DRAFT spec/42) | Outcome state persistence; `FilesystemOutcomeBackend` reference impl; byte-identical on-disk `result.json` + portable `export()` with relative artifact refs; `OutcomeExport` wired into `Exportable` (spec/40); write-once/result-immutability; the fifteenth backend Protocol | [`spec/42`](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/spec/42-outcome-backend.md) |

**v1 direction:** a home user runs filesystem-everything today. An organization runs the same agent definitions over Postgres / Redis / SQLite-Datadog / behind an HTTP service. v1.0 is here: all 12 backend protocols shipped and locked, conformance suites pin the contracts. A thirteenth, `SecretBackend` (#340), shipped for v1.5 (both PRs merged, spec/38 LOCKED, Filesystem + GCP Secret Manager reference impls). A fourteenth, `GoalBackend` (#425), shipped for v1.5 (DRAFT spec/41, `FilesystemGoalBackend` reference impl). A fifteenth, `OutcomeBackend` (#426), shipped for v1.5 PR 1 (DRAFT spec/42, `FilesystemOutcomeBackend` reference impl). See [`docs/architecture.md`](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/architecture.md) for the mental model, [`docs/TENSIONS.md`](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/TENSIONS.md) for architectural tensions this scaling story has to survive, and [`ROADMAP.md`](https://github.com/dep0we/atomic-agents-stack/blob/main/ROADMAP.md) for the full backlog beyond v1.0.
...
**v1.1.0, stable.** Core runtime stable. Twelve of twelve backend protocols shipped and locked at v1.0 (see the backend protocols table above); `MCPServerRegistryBackend` LOCKED at PR 5 v1.0. A thirteenth, `SecretBackend` (#340), shipped for v1.5 (both PRs merged, spec/38 LOCKED, Filesystem + GCP Secret Manager reference impls). A fourteenth, `GoalBackend` (#425), shipped for v1.5 (DRAFT spec/41, `FilesystemGoalBackend` reference impl). A fifteenth, `OutcomeBackend` (#426), shipped for v1.5 PR 1 (DRAFT spec/42, `FilesystemOutcomeBackend` reference impl). The v1.0 Protocol surface is stable per SemVer Major. See [`docs/deployment/versioning.md`](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/deployment/versioning.md) for the breaking-change policy. Single-maintainer project; reference implementation anyone can use, fork, or extend.
