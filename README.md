...
**v1.1.0, stable.** Core runtime stable. Twelve of twelve backend protocols shipped and locked at v1.0 (see the backend protocols table above); `MCPServerRegistryBackend` LOCKED at PR 5 v1.0. A thirteenth, `SecretBackend` (#340), shipped for v1.5 (both PRs merged, spec/38 LOCKED, Filesystem + GCP Secret Manager reference impls). A fourteenth, `GoalBackend` (#425), shipped for v1.5 (DRAFT spec/41, `FilesystemGoalBackend` reference impl). A fifteenth, `OutcomeBackend` (#426), shipped for v1.5 PR 1 (DRAFT spec/42, `FilesystemOutcomeBackend` reference impl). A sixteenth, `JournalBackend` (#427), shipped for v1.5 PR 1 (DRAFT spec/43, `FilesystemJournalBackend` reference impl, ADOPT-NOW wiring). The v1.0 Protocol surface is stable per SemVer Major. See [`docs/deployment/versioning.md`](https://github.com/dep0we/atomic-agents-stack/blob/main/docs/deployment/versioning.md) for the breaking-change policy. Single-maintainer project; reference implementation anyone can use, fork, or extend.
